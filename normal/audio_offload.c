...
static int audio_offload_get_dev_config(struct tiny_audio_device *adev,uint32_t *pcard,uint32_t *pdevice, struct pcm_config *pcm_cfg)
{
    ...
    ALOGI("%s,card: %d, port: %d,adev->call_start: %d,adev->voip_state: %d,p_c:%d,p_size: %d,reate:%d,channel:%d",__func__,
...
static int audio_get_offload_codec_id(audio_format_t format)
{
    ...
    case AUDIO_FORMAT_MP3:
        ...
        break;
    default:
        ALOGW("%s: audio format (%d) is not supported ", __func__,format);
    }
...
static int audio_start_compress_output(struct tiny_stream_out *out)
{
    ...
    ALOGI("%s: vbc _switch is %d", __func__, mixer_ctl_get_value(adev->private_ctl.vbc_switch,0));
    if(out->compress) {
    ...
    ALOGI("%s: successfully, out ", __func__);
    return 0;
error:
    ALOGE("%s: error is_offload_running: %d",__func__,adev->is_offload_running);
    if(out->compress) {
...
static void *audio_offload_thread_loop(void *param)
{
    ...
    for (;;) {
        ...
        ALOGI("%s, offload_cmd_list:%d, offload_state:%d",
        ...
        case AUDIO_OFFLOAD_CMD_PARTIAL_DRAIN:
            ALOGI("keokeo before AUDIO_OFFLOAD_CMD_PARTIAL_DRAIN");
            compress_next_track(out->compress);
            compress_partial_drain(out->compress);
            ALOGI("keokeo after AUDIO_OFFLOAD_CMD_PARTIAL_DRAIN");
            need_send_callback = true;
            event = STREAM_CBK_EVENT_DRAIN_READY;
            break;
        case AUDIO_OFFLOAD_CMD_DRAIN:
            ALOGI("keokeo before AUDIO_OFFLOAD_CMD_DRAIN");
            compress_drain(out->compress);
            ALOGI("keokeo after AUDIO_OFFLOAD_CMD_DRAIN");
            need_send_callback = true;
            event = STREAM_CBK_EVENT_DRAIN_READY;
            break;
        default:
...
{
    pthread_mutex_lock(&out->lock);
    if (out->is_offload_compress_started && out->compress != NULL)
        audio_offload_mute(out);
...
{
#ifdef AUDIOHAL_V4
    FILE *fp = fopen("/data/vendor/local/media/leodump.wav","ab");
#else
        ...
        size_t bytes)
{
    int ret = 0;
    struct tiny_audio_device *adev = out->dev;
    ALOGD("%s: want to write buffer (%d bytes) to compress device, offload_state:%d ",
    ...
    if (out->is_offload_need_set_metadata) {
        ALOGI("%s: need to send new metadata to driver ",__func__);
    ...
    ALOGD("%s: finish writing buffer (%d bytes) to compress device, and return %d",
            __func__, bytes, ret);
            ...
            stream_callback_t callback, void *cookie)
{
    ...
    ALOGD("%s in, audio_app_type:%d ", __func__, out->audio_app_type);
    pthread_mutex_lock(&out->lock);
