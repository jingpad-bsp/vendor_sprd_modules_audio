...
#ifdef AUDIOHAL_V4
#define FILE_PATH_MUSIC  "/data/vendor/local/media/audio_dumpmusic.pcm"
#define FILE_PATH_SCO  "/data/vendor/local/media/audio_dumpsco.pcm"
#define FILE_PATH_BTSCO  "/data/vendor/local/media/audio_dumpbtsco.pcm"
#define FILE_PATH_VAUDIO  "/data/vendor/local/media/audio_dumpvaudio.pcm"
#define FILE_PATH_INREAD  "/data/vendor/local/media/audio_dumpinread.pcm"
#define FILE_PATH_INREAD_AFTER_PROCESS  "/data/vendor/local/media/audio_dumpinread_noprocess.pcm"
#define FILE_PATH_INREAD_AFTER_RESAMPLER  "/data/vendor/local/media/audio_dumpinread_aftersrc.pcm"
#define FILE_PATH_INREAD_AFTER_NR  "/data/vendor/local/media/audio_dumpinread_afternr.pcm"

#define FILE_PATH_INREAD_VBC  "/data/vendor/local/media/audio_dumpinread_vbc.pcm"
#define FILE_PATH_MUSIC_WAV  "/data/vendor/local/media/audio_dumpmusic.wav"
#define FILE_PATH_SCO_WAV  "/data/vendor/local/media/audio_dumpsco.wav"
#define FILE_PATH_BTSCO_WAV  "/data/vendor/local/media/audio_dumpbtsco.wav"
#define FILE_PATH_VAUDIO_WAV  "/data/vendor/local/media/audio_dumpvaudio.wav"
#define FILE_PATH_INREAD_WAV  "/data/vendor/local/media/audio_dumpinread.wav"
#define FILE_PATH_INREAD_AFTER_PROCESS_WAV  "/data/vendor/local/media/audio_dumpinread_noprocess.wav"
#define FILE_PATH_INREAD_AFTER_SRC_WAV  "/data/vendor/local/media/audio_dumpinread_aftersrc.wav"
#define FILE_PATH_INREAD_AFTER_NR_WAV  "/data/vendor/local/media/audio_dumpinread_afternr.wav"
#define FILE_PATH_INREAD_VBC_WAV  "/data/vendor/local/media/audio_dumpinread_vbc.wav"

#define FILE_PATH_HAL_INFO  "/data/vendor/local/media/audio_hw_info.txt"
#define FILE_PATH_HELP  "/data/vendor/local/media/help.txt"
#define FILE_PATH_CP_RAM_PRE  "/data/vendor/local/media/voicepoint"
#define MAX_FILE_LENGTH 255
#define FILE_PATH_CP_ENABLE_POINT_INFO  "/data/vendor/local/media/cppoint.txt"
#define MMI_DEFAULT_PCM_FILE  "/data/vendor/local/media/aploopback.pcm"
#define MMI_LOOP_PCM_FILE  "/data/vendor/local/media/loop.pcm"
#define AUDIO_TEST_CP_LOOP_PLAY_PCM_FILE "/data/vendor/local/media/rx_data.pcm"
#define AUDIO_TEST_CP_LOOP_RECD_PCM_FILE "/data/vendor/local/media/tx_data.pcm"
#define AUDIO_EXT_DATA_CONTROL_PIPE "/data/vendor/local/media/mmi.audio.ctrl"
#define AUDIO_LOOP_TEST_RESULT_PROPERITY  "persist.vendor.audioloop.test.result"
#else
...
void *in_test_thread(void *args){
    ...
    pcm = pcm_open(card, 0, PCM_IN|PCM_MMAP|PCM_NOIRQ|PCM_MONOTONIC , &in_test->config.config);
#else
    ...
    process_init = init_rec_process(GetAudio_InMode_number_from_device(in_test->dev_test->adev, in_test->dev_test->adev->in_devices),
...
int ext_control_close (struct ext_control_mgr *pEntryMgr)
{
    ...
    if(pEntryMgr->isExit==true){
        ALOGI("ext_control_close is Exit");
        return 0;
    }

    if(access(AUDIO_EXT_CONTROL_PIPE, R_OK) ==0){
        pEntryMgr->fd = open(AUDIO_EXT_CONTROL_PIPE, O_WRONLY);
    }else{
        pEntryMgr->fd = open(AUDIO_EXT_DATA_CONTROL_PIPE, O_WRONLY);
    }
...
int create_ext_pipe(void){
    ...
    if (mkfifo(MMI_DEFAULT_PCM_FILE,S_IFIFO|0666) <0) {
        if (errno != EEXIST) {
            ...
            ALOGI("%s exit line:%d",__func__,__LINE__);
            return -1;
        }
    }
    ...
    if(access(AUDIO_EXT_CONTROL_PIPE, R_OK) != 0){
        if(access(AUDIO_EXT_DATA_CONTROL_PIPE, R_OK) != 0){
            ALOGI("%s create audio fifo line:%d\n",__FUNCTION__,__LINE__);
            ret=mkfifo(AUDIO_EXT_DATA_CONTROL_PIPE,S_IFIFO|0666);
            ALOGI("%s create audio fifo line:%d ret:%d\n",
                __FUNCTION__,__LINE__,ret);
            while((ret!=0)&&(try_count>0)){
                ALOGE("%s create audio fifo try:%d ret:%d error  %s\n",
            ...
            if(0==ret){
                ALOGI("%s create audio fifo success\n",__FUNCTION__);
            }else{
                ALOGE("%s create audio fifo try:%d error: %s\n",
    ...
    if(access(AUDIO_EXT_DATA_CONTROL_PIPE, R_OK) == 0){
        ...
        ALOGI("%s set audio fifo mode\n",__FUNCTION__);
        if(chmod(AUDIO_EXT_DATA_CONTROL_PIPE, S_IRUSR|S_IWUSR|S_IRGRP|S_IWGRP|S_IROTH|S_IWOTH) != 0) {
            ALOGE("%s Cannot set RW to \"%s\": %s", __FUNCTION__,AUDIO_EXT_DATA_CONTROL_PIPE, strerror(errno));
        }

        while(try_count>0){
            ALOGI("Check pipe mode try:%d",try_count);
            try_count--;
            memset(&buf,0,sizeof(struct stat));
            if(stat(AUDIO_EXT_DATA_CONTROL_PIPE, &buf)!=0){
                ALOGE("%s stat audio fifo error %s\n",__FUNCTION__,strerror(errno));
                break;
            }

            if((buf.st_mode&(S_IRUSR|S_IWUSR|S_IRGRP|S_IWGRP|S_IROTH|S_IWOTH))
                !=(S_IRUSR|S_IWUSR|S_IRGRP|S_IWGRP|S_IROTH|S_IWOTH)){
                ALOGI("create_ext_pipe pipe mode error:0x%x",buf.st_mode);
                if(chmod(AUDIO_EXT_DATA_CONTROL_PIPE, S_IRUSR|S_IWUSR|S_IRGRP|S_IWGRP|S_IROTH|S_IWOTH) != 0) {
                    ALOGE("%s Cannot set RW to \"%s\": %s", __FUNCTION__,AUDIO_EXT_DATA_CONTROL_PIPE, strerror(errno));
                }
                usleep(100*1000);
            }else{
                ALOGI("create_ext_pipe pipe mode check success:0x%x",buf.st_mode);
                try_count=0;
                break;
            }
        }
    }else{
        ALOGI("create_ext_pipe can not access:%s",AUDIO_EXT_DATA_CONTROL_PIPE);
    }
    return 0;
}
int ext_control_open(struct tiny_audio_device *adev){
    ...
    if(pthread_create(&adev->ext_contrl->entryMgr.thread_id, &attr, control_audio_loop_process, (void *)adev)) {
...
void check_ext_control(struct tiny_audio_device *adev){
    int ret=0;
    if(adev->ext_contrl->entryMgr.isExit = true){
        ret = ext_control_open(adev);
        if (ret)  ALOGW("check_ext_control: audio ext_contrl can NOT work.");
    }
}
...
static int string2intarray(char* string,int* pointarray,int arraysize){
    ...
    if(strlen(string) != 0){
        firstnum = string;
        while((firstnum - string) < strlen(string)){
            char c = *firstnum;
            if(c <='9'&& c >= '0'){
                isnum = 1;
                break;
...
static int ext_get_dump_data_switch(void) {
    ...
    int dump_switch = 0;
#ifdef AUDIOHAL_V4
    rc = ext_property_get("persist.vendor.media.dump.switch", value, NULL);
#else
...
static void *control_audio_loop_process(void *arg){
    ...
    while(!adev->ext_contrl->entryMgr.isExit && adev != NULL){
...
#ifdef AUDIOHAL_V4
            ext_property_set("persist.vendor.media.dump.path", value);
#else
        ...
        if(ret >= 0){
            ...
            ext_property_set(DUMP_AUDIO_PROPERTY, value);
            adev->dump_flag=strtoul(value,NULL,0);
            ALOGI("update dump_flag:0x%x",adev->dump_flag);
        }
