...
static int source_in_read(struct audio_test_point *source,struct test_endpoint_param *param, void *buffer, int size, int mode){
...
static void *audio_source_thread(void *args){
    ...
    while(source->is_exit==false){
        ...
        ret=source_in_read(source,&source->param, buffer, source->period_size,mode);
        if((ret< source->period_size)&&(source->param.fd>0)){
            lseek(param->fd,0,SEEK_SET);
        }
        if((mode==1)&&(ret< source->period_size)&&(source->param.fd>0)){
            if(loopcunt>0){
                loopcunt--;
            }
            if(loopcunt<=0){
                ALOGI("audio_source_thread start stop test with loop end");
                source->is_exit=true;
            }
        }else if((mode==2)&&(stoptime< getCurrentTimeUs())){
            ALOGI("audio_source_thread start stop test with timeout");
            source->is_exit=true;
        }
        pthread_mutex_lock(&(source->lock));
    }
    ...
    if(mode!=0){
        ...
        sink->is_exit=true;
        ALOGI("audio_source_thread stop sink thread");
...
static void dump_point(struct audio_test_point *point){
    ...
    ALOGI("point:%p is_exit:%d stream:%p period_size:%d",
        point,point->is_exit,point->stream,point->period_size);

    if(point->ring_buf!=NULL){
        ALOGI("point:%p ring_buf:%p ring_buf size:%d in:%d out:%d",
...
void set_hidl_read_size(void *dev,int size){
    ...
    if(sink!=NULL){
        ...
        ALOGI("set_hidl_read_size npi_readsize:%d",sink->npi_readsize);
    }else{
        ALOGE("set_hidl_read_size failed");
    }
    pthread_mutex_unlock(&(sink->lock));
}
...
int hidl_stream_read(void *dev, void *buffer,int size){
    ...
    if(sink->npi_readsize>0){
        ...
        if(sink==NULL){
            ALOGW("hidl_stream_read Failed sink is null");
            return 0;
        }

        do{
            ret=ring_buffer_get(sink->ring_buf, (void *)buffer_tmp, total_read);
            ALOGI("hidl_stream_read ring_buffer_get:%d %d",total_read,ret);
            total_read-=ret;
            buffer_tmp+=ret;
            if(total_read){
                usleep(10*1000);
                ALOGI("hidl_stream_readtotal_read:%d < size:%d",total_read,size);
                if(ret<=0){
                    read_count--;
                    if(read_count<=0){
                        break;
...
static int read_record_data(struct audio_test_point *sink,int size){
    ...
    do{
        ...
        buffer_tmp+=ret;
        usleep(10*1000);
        ALOGI("read_record_data total_read:%d < size:%d",sink->npi_readsize,size);
...
static int start_endpoint_test(void * dev,struct endpoint_test_control *ctl,
    ...
    ctl->written=0;
     ...
     if(ctl->hild_outputstream!=NULL){
         ...
         if(stream_out->devices){
            ...
            ALOGI("start_endpoint_test line:%d device:0x%x %p",__LINE__,adev->out_devices);
        }
     }
#endif
    ...
    get_pcm_config(ctl->sink,&config,param);
     ...
     if(ctl->hidl_inputstream!=NULL){
         ...
         ALOGI("start_endpoint_test line:%d device:0x%x",__LINE__,adev->in_devices);
            if(stream_in->device){
            ALOGI("start_endpoint_test line:%d device:0x%x",__LINE__,adev->in_devices);
            adev->in_devices=stream_in->device;
            ALOGI("start_endpoint_test line:%d device:0x%x",__LINE__,adev->in_devices);
        }
     }
#endif
...
int write_endpoint_outputtest_buffer(struct endpoint_control *ctl, const void* buffer,
    ...
    int sprdhidl_writesize=ctl->ouput_test.hidl_buffer_size;
        ...
        if(bytes>=ctl->ouput_test.hidl_buffer_size-ctl->ouput_test.written){
...
int audio_endpoint_test(void * dev,struct str_parms *parms,char * val){
    ...
    if(test_param->type==OUTPUT_ENDPOINT_TEST){
        ...
        if(adev->test_ctl.ouput_test.hild_outputstream!=NULL){
            ...
            test_param->pcm_channels=audio_channel_count_from_out_mask(hidlstream_out->channel_mask);
            test_param->pcm_rate=hidlstream_out->sample_rate;
