...
#ifdef AUDIOHAL_V4
#define SMART_MONITOR_FILE_1            "/data/vendor/local/media/smart_amp_monitor_1.bin"
#define SMART_MONITOR_FILE_2            "/data/vendor/local/media/smart_amp_monitor_1.bin"
#else
    ...
    if (tmpSize >=  SMART_AMP_PROCESS_LENGTH) { /*use catch and argin*/
        ...
        for (loop = 0; loop < totalSize / SMART_AMP_PROCESS_LENGTH; loop ++) {
            if (pCtl->flagDump) {
                ...
                if (NULL == before_file) {
#ifdef AUDIOHAL_V4
                    before_file = fopen("/data/vendor/local/media/dump_smart_amp_process_before.pcm", "wb");
#else
            ...
            if (pCtl->flagDump) {
                ...
                if (NULL == after_file) {
#ifdef AUDIOHAL_V4
                    after_file = fopen("/data/vendor/local/media/dump_smart_amp_process_after.pcm", "wb");
#else
