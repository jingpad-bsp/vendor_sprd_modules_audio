...
#ifdef ANDROID6X
/*define invaild magic number for AUDIO_SOURCE_UNPROCESSED Android 6.0*/
#define AUDIO_SOURCE_UNPROCESSED  0xabbabaab
#endif
...
static int32_t GetAudio_InMode_number_from_device(struct tiny_audio_device * adev, int in_dev)
{
    int ret = 3;
    ...
    if (NULL != adev->active_input) {
        if (AUDIO_SOURCE_UNPROCESSED == adev->active_input->source) {
            if (AUDIO_MODE_UNPROCESS > adev_get_audiomodenum4eng()) {
                 ALOGI("%s, Audio Source is Unprocessed, but NV don't support Unprocessed, so Mode:%d", __func__, ret);
            } else {
                ret = AUDIO_MODE_UNPROCESS;
                ALOGI("%s, Audio NV mode is unprocess now! mode:%d", __func__, ret);
            }
        }
        if (AUDIO_SOURCE_VOICE_RECOGNITION == adev->active_input->source) {
            if (AUDIO_MODE_RECOGNITION >= adev_get_audiomodenum4eng())
                ALOGI("%s, Audio Source is Recognition, but NV don't support Recognition, so Mode:%d", __func__, ret);
            else {
                ret = AUDIO_MODE_RECOGNITION;
                ALOGI("%s, Audio NV mode is Recognition now! mode:%d", __func__, ret);
           }
        }
    }
...
static int  GetAudio_PaConfig_nv(struct tiny_audio_device *adev, AUDIO_TOTAL_T *aud_params_ptr, pga_gain_nv_t *pga_gain_nv)
{
    ...
    ALOGI("GetAudio_PaConfig_nv hp_pa_delay_config 0x%x pa_config:0x%x(0x%x, %x), 0x%x(fm)(0x%x, %x),  hpPaConfig:0x%x(0x%x, %x), 0x%x(fm)(0x%x, %x)",
...
static int GetAudio_gain_by_devices(struct tiny_audio_device *adev, pga_gain_nv_t *pga_gain_nv, uint32_t vol_level,int out_dev,int in_dev)
{
    ...
    inmode = GetAudio_InMode_number_from_device(adev, in_dev);

    ALOGI("%s, outmode:%d inmode:%d,indev:0x%x, outdev:0x%x",
...
static int SetVoice_PaConfig_by_devices(struct tiny_audio_device *adev, pga_gain_nv_t *pga_gain_nv)
{
    ...
    ALOGI("%s out, out_devices:0x%x, in_devices:0x%x", __func__, pga_gain_nv->out_devices, pga_gain_nv->in_devices);
    return 0;
}
...
static int SetAudio_PaConfig_by_devices(struct tiny_audio_device *adev, pga_gain_nv_t *pga_gain_nv)
{
    ...
    if((adev->fm_open) && (mode != 1)) {
        ...
        ALOGI("%s FM out, out_devices:0x%x, in_devices:0x%x", __func__,pga_gain_nv->out_devices, pga_gain_nv->in_devices);
        return 0;
    }
...
static void GetCall_VolumePara(struct tiny_audio_device *adev,paras_mode_gain_t *mode_gain_paras)
{
    ...
    ALOGI("SetCall_VolumePara hp_pa_config: 0x%x, 0x%x, 0x%x,pa_config: 0x%x, 0x%x, 0x%x.",
    ...
    ALOGI("%s successfully ,dac_pga_gain_l:0x%x ,dac_pga_gain_r:0x%x ,adc_pga_gain_l:0x%x ,adc_pga_gain_r:0x%x, devices:0x%x ,mode:%d ",
...
static void SetCall_ModePara(struct tiny_audio_device *adev,paras_mode_gain_t *mode_gain_paras)
{
    ...
    ALOGI("%s successfully, device: earpice(%s), headphone(%s), speaker(%s), Main_Mic(%s), Back_Mic(%s), hp_mic(%s), mode_indxe: %#x"
...
int SetParas_OpenHal_Incall(struct tiny_audio_device *adev, int fd_pipe)	//Get open hal cmd and sim card
{
    ...
    if (android_sim_num & 0x1000) {
        android_sim_num &= ~0x1000;
        adev->loopback_start = 1;
        ALOGI("%s,voice loopback mode start!\n", __func__);
    }
...
int vbc_ctrl_open(struct tiny_audio_device *adev)
{
...
#ifndef DYNAMIC_AUDIO_ENCRYPT
#ifdef AUDIOHAL_V4
    if(access("/data/vendor/local/media/vb_voice_fake_pipe", R_OK) != 0){
        if (mkfifo("/data/vendor/local/media/vb_voice_fake_pipe",S_IFIFO|0666) <0) {
            if (errno != EEXIST) {
        ...
        if(chmod("/data/vendor/local/media/vb_voice_fake_pipe", S_IRUSR|S_IWUSR|S_IRGRP|S_IWGRP|S_IROTH|S_IWOTH) != 0) {
            LOG_E("%s Cannot set RW to \"%s\": %s", __FUNCTION__,"vb_voice_fake_pipe", strerror(errno));
        }
    }

    if (vb_voice_fake_pipe < 0) {
        vb_voice_fake_pipe = open("/data/vendor/local/media/vb_voice_fake_pipe", O_RDWR);
    }
#else
    ...
    {
#ifdef DYNAMIC_AUDIO_ENCRYPT
        vbc_ctrl_thread_para_t *thread_para=(vbc_ctrl_thread_para_t *)(st_vbc_ctrl_thread_para+i);
        if (pipe(thread_para->pipe_fd) < 0) {
          ALOGE("vbc_ctrl_open create pipe failed");
        }
#endif
...
{
#ifdef DYNAMIC_AUDIO_ENCRYPT
    vbc_ctrl_thread_para_t *thread_para=NULL;
#endif
    ...
    {
#ifdef DYNAMIC_AUDIO_ENCRYPT
        thread_para=(vbc_ctrl_thread_para_t *)(st_vbc_ctrl_thread_para+i);
#endif

        st_vbc_ctrl_thread_para[i].isExit = true;
        thread_id = s_vbc_ctrl_thread_id[i];
#ifdef DYNAMIC_AUDIO_ENCRYPT
       if(thread_para->pipe_fd[1]>0){
           w_size = write(thread_para->pipe_fd[1], fake, sizeof(fake));
       }else{
           w_size=0;
       }
#else
        ...
        i++;
#ifdef DYNAMIC_AUDIO_ENCRYPT
        if(thread_para->pipe_fd[0]>0){
            close(thread_para->pipe_fd[0]);
        }
        thread_para->pipe_fd[0]=-1;

        if(thread_para->pipe_fd[1]>0){
            close(thread_para->pipe_fd[1]);
        }
        thread_para->pipe_fd[1]=-1;
#endif
    }
...
#ifdef DYNAMIC_AUDIO_ENCRYPT
    if(res->pipe_fd[1]>0){
        w_size = write(res->pipe_fd[1], fake, sizeof(fake));
        if (w_size  > 0) {
...
int vbc_ctrl_voip_open(struct voip_res *res)
{
   ...
   if(!res->enable){
    ...
    if (pipe(res->pipe_fd) < 0) {
      ALOGE("pipe_fd create pipe failed");
    }
#else
#ifdef AUDIOHAL_V4
    if(access("/data/vendor/local/media/vb_voip_fake_pipe", R_OK) != 0){
        if (mkfifo("/data/vendor/local/media/vb_voip_fake_pipe",S_IFIFO|0666) <0) {
            if (errno != EEXIST) {
                LOG_E("%s create audio fifo error %s\n",__FUNCTION__,strerror(errno));
                return -1;
            }
        ...
        if(chmod("/data/vendor/local/media/vb_voip_fake_pipe", S_IRUSR|S_IWUSR|S_IRGRP|S_IWGRP|S_IROTH|S_IWOTH) != 0) {
            LOG_E("%s Cannot set RW to \"%s\": %s", __FUNCTION__,"vb_voip_fake_pipe", strerror(errno));
        }
    }

    if (vb_voip_fake_pipe < 0) {
        vb_voip_fake_pipe = open("/data/vendor/local/media/vb_voip_fake_pipe", O_RDWR);
    }
#else
...
void *vbc_ctrl_voip_thread_routine(void *arg)
{
    ...
    prctl(PR_SET_NAME, (unsigned long)"Vbc Ctrl Voip Thread", 0, 0, 0);
...
#ifdef DYNAMIC_AUDIO_ENCRYPT
    vb_voip_fake_pipe=res->pipe_fd[0];
    if(vb_voip_fake_pipe<=0){
        ALOGW("vbc_ctrl_voip_thread_routine pipe is not ready");
...
#ifdef DYNAMIC_AUDIO_ENCRYPT
        if((vb_voip_fake_pipe>0)&&(FD_ISSET(vb_voip_fake_pipe,&fds_read)))
#else
        ...
        switch (read_common_head.cmd_type)
        {
            ...
            case VBC_CMD_SET_MODE:
            {
                ...
                if(adev->smart_pa_exist && !(s_call_dl_devices & AUDIO_DEVICE_OUT_SPEAKER))
                    vbc_ctrl_nxp_tfa_close(adev);
#endif
                parameters_head_t write_voltage_head = {0};
                long int voltage = 0;
                //get battery voltage(uv)
                ret = smart_amp_get_battery_voltage(&voltage, BATTERY_VOLTAGE_PATH);
                if (ret < 0) {
                    ALOGE("voip1:get battery voltage error!\n");
                    voltage = 0;
                } else {
                    ALOGE("voip1:batteryVoltage=%ld\n",voltage);
                }
                memcpy(write_voltage_head.tag,VBC_CMD_TAG,3);
                write_voltage_head.cmd_type = VBC_CMD_RSP_MODE;
                write_voltage_head.paras_size = voltage/1000;
                ALOGE("voip1:voltage=%d\n",write_voltage_head.paras_size);
                ret = WriteParas_Head(para->vbpipe_fd,&write_voltage_head);
                if(ret < 0){
                    ALOGE("voip1:Error,SET_MODE Write_Rsp2cp1 failed(%d).",ret);
                }
#ifdef AUDIO_USERDEBUG
                dump_all_audio_reg(-1,0x8);
#endif
...
void *vbc_ctrl_thread_routine(void *arg)
{
    ...
    setpriority(PRIO_PROCESS, 0, ANDROID_PRIORITY_AUDIO);
    set_sched_policy(0, SP_FOREGROUND);
    prctl(PR_SET_NAME, (unsigned long)"Vbc Ctrl Thread", 0, 0, 0);
...
RESTART:
...
#ifdef DYNAMIC_AUDIO_ENCRYPT
    vb_voice_fake_pipe=para->pipe_fd[0];
    if(vb_voice_fake_pipe<=0){
    ...
    while(!para->isExit)
    {
        ...
        if(FD_ISSET(para->vbpipe_fd,&fds_read) <= 0) {
#ifdef DYNAMIC_AUDIO_ENCRYPT
            if((vb_voice_fake_pipe>0)&&(FD_ISSET(vb_voice_fake_pipe,&fds_read)))
#else
	...
	case VBC_CMD_SET_MODE:
		...
		ret = smart_amp_get_battery_voltage(&voltage, BATTERY_VOLTAGE_PATH);
		if (ret < 0) {
			ALOGE("voice:get battery voltage error!\n");
			voltage = 0;
		} else {
			ALOGE("voice:batteryVoltage=%ld\n",voltage);
		}
		memcpy(write_voltage_head.tag,VBC_CMD_TAG,3);
		write_voltage_head.cmd_type = VBC_CMD_RSP_MODE;
		write_voltage_head.paras_size = voltage/1000;
		ALOGE("voice:voltage=%d\n",write_voltage_head.paras_size);
		ret = WriteParas_Head(para->vbpipe_fd,&write_voltage_head);
		if(ret < 0){
...
static void * vbc_ctl_modem_monitor_routine(void *arg)
{
    ...
    setpriority(PRIO_PROCESS, 0, ANDROID_PRIORITY_AUDIO);
    set_sched_policy(0, SP_FOREGROUND);
    prctl(PR_SET_NAME, (unsigned long)"Vbc Ctl Modem Thread", 0, 0, 0);
...
reconnect:
...
#ifdef DYNAMIC_AUDIO_ENCRYPT
       if(adev->modem_monitor_mgr.is_entry_exit==true){
            break;
       }
#endif
    } while(fd < 0);
#ifdef DYNAMIC_AUDIO_ENCRYPT
    adev->modem_monitor_mgr.fake_fd=adev->modem_monitor_mgr.pipe_fd[0];
    if(adev->modem_monitor_mgr.fake_fd<=0){
        maxfd = fd + 1;
        ALOGW("%s line:%d pipe is not ready",__func__,__LINE__);
    }else{
        if((fcntl(adev->modem_monitor_mgr.fake_fd,F_SETFL,O_NONBLOCK))<0)
        {
            ALOGD("vbc_ctl_modem_monitor_routine fcntl %d error.",
                adev->modem_monitor_mgr.fake_fd);
        }
...
#ifdef DYNAMIC_AUDIO_ENCRYPT
            if(((adev->modem_monitor_mgr.fake_fd>0))&&(FD_ISSET(adev->modem_monitor_mgr.fake_fd,&fds_read)))
#else
    ...
    close(fd);
    ALOGI("vbc_ctl_modem_monitor_routine Exit");
    return 0;
}
...
#ifdef DYNAMIC_AUDIO_ENCRYPT
    w_size = write(adev->modem_monitor_mgr.pipe_fd[1], fake, sizeof(fake));
    if (w_size > 0) {
        pthread_join(adev->modem_monitor_mgr.thread_id, (void **) NULL);
    }

    if(adev->modem_monitor_mgr.pipe_fd[0]>0){
        close(adev->modem_monitor_mgr.pipe_fd[0]);
    }
    adev->modem_monitor_mgr.pipe_fd[0]=-1;

    if(adev->modem_monitor_mgr.pipe_fd[1]>0){
        close(adev->modem_monitor_mgr.pipe_fd[1]);
    }
    adev->modem_monitor_mgr.pipe_fd[1]=-1;
#else
...
int vb_ctl_modem_monitor_open(void * arg)
{
    ...
    adev->modem_monitor_mgr.fake_fd = -1;
#ifdef DYNAMIC_AUDIO_ENCRYPT
    if (pipe(adev->modem_monitor_mgr.pipe_fd) < 0) {
      ALOGE("vb_ctl_modem_monitor_open create pipe failed");
    }
    adev->modem_monitor_mgr.fake_fd = adev->modem_monitor_mgr.pipe_fd[0];
#else
#ifdef AUDIOHAL_V4
    if(access("/data/vendor/local/media/vb_monitor_fake_pipe", R_OK) != 0){
        if (mkfifo("/data/vendor/local/media/vb_monitor_fake_pipe",S_IFIFO|0666) <0) {
            if (errno != EEXIST) {
                LOG_E("%s create audio fifo error %s\n",__FUNCTION__,strerror(errno));
                return -1;
            }
        ...
        if(chmod("/data/vendor/local/media/vb_monitor_fake_pipe", S_IRUSR|S_IWUSR|S_IRGRP|S_IWGRP|S_IROTH|S_IWOTH) != 0) {
            LOG_E("%s Cannot set RW to \"%s\": %s", __FUNCTION__,"vb_monitor_fake_pipe", strerror(errno));
        }
    }

    do {
        fd = open("/data/vendor/local/media/vb_monitor_fake_pipe", O_RDWR);
        if (fd < 0) {
