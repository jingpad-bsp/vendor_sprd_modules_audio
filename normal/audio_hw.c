...
#ifdef AUDIOHAL_V4
#define AUDFIFO_2 "/data/vendor/local/media/audiopara_tuning_2"
#else
#include "eng_audio.h"
#endif
#include "aud_proc.h"
...
#ifdef AUDIOHAL_V4
#define AUDIO_OUT_FILE_PATH  "/data/vendor/local/media/audio_out.pcm"
#else
...
#ifdef AUDIOHAL_V4
#define FM_DIGITAL_SUPPORT_PROPERTY  "ro.vendor.digital.fm.support"
#else
...
#define MIXER_TO_VOICE_UPLINK_ENABLE   0x6
#define MIXER_TO_VOICE_UPLINK_DISABLE  0x7
...
#ifdef AUDIOHAL_V4
#define AUDFIFO "/data/vendor/local/media/audiopara_tuning"
#else
...
struct pcm_config pcm_config_mm_fast = {
    ...
    .period_count = PLAYBACK_SHORT_PERIOD_COUNT,
    .format = PCM_FORMAT_S16_LE,
    .start_threshold = SHORT_PERIOD_SIZE * PLAYBACK_SHORT_PERIOD_COUNT/2,
...
struct tiny_audio_device {
    ...
    struct tiny_stream_in *active_fmul_input;
    ...
    struct tiny_stream_out *active_voicetx_output;
...
struct tiny_private_ctl private_ctl;
    ...
    bool is_debug_btsco;
    bool boot_completed;
    voip_timer_t bootup_timer; //for bootup
    int record_bypass;
    int fm_spk_bypass;
#ifdef AUDIOHAL_V4
...
struct tiny_stream_out {
    ...
    unsigned int apm_devices;//only modify by audiopolicymanager
    int write_threshold;
    bool low_power;
    FILE * out_dump_fd;
...
#define AUDIO_DEVICE_OUT_ALL_FM AUDIO_DEVICE_OUT_FM_HEADSET|AUDIO_DEVICE_OUT_FM_SPEAKER
#endif
...
#ifdef FM_VOLUME_LEVEL_26
#define FM_VOLUME_MAX 26
#else
...
enum{
    ...
    AUDIO_MODE_UNPROCESS = 6,
    AUDIO_MODE_RECOGNITION = 7,
};
...
extern int aud_misc_ctl_disable_pownerhint(void * misc);
extern int aud_misc_ctl_enable_pownerhint(void * misc);
...
extern int usb_card_parse(char * namebuf, int namebuf_len);
extern int usb_pdev_parse(int32_t card);
...
static int do_speaker_mute(struct tiny_audio_device *adev , bool mute);
...
static void vbc_ad01iis_select(struct tiny_audio_device *adev,
    bool is_to_dfm);
static int set_fm_mute_noramp(struct audio_hw_device *dev, bool mute);
...
static int init_vbc_eq(void * dev);
static bool check_vbc_eq(void *dev);
void check_ext_control(struct tiny_audio_device *adev);
#ifdef DYNAMIC_AUDIO_ENCRYPT
static bool check_data_ready(void *dev);
#endif
#ifdef NXP_SMART_PA
extern void register_nxp_pcmdump(nxp_pa_handle handle,void *fun);
#endif
...
static bool get_boot_status(void){
    ...
    if (strncmp("1", bootvalue, 1) != 0) {
        ALOGI("get_boot_status false");
        return false;
    }else{
        ALOGI("get_boot_status true");
        return true;
    }
#endif
}
...
static void check_headset_status(struct tiny_stream_out *out){
    ...
    int devices=out->devices;
    int pre_devices=out->devices;
    ...
    if(!list_empty(&adev->active_out_list)){
        list_for_each_safe(item, item2,&adev->active_out_list){
            ...
            if((out!=NULL)
                ...
                ALOGI("do_bootcomplated update normal output devices:0x%x",out->devices);
                select_devices_signal(adev);
                break;
            }
        }
...
static void check_boot_status(void *dev){
    ...
    if(false==adev->boot_completed){
        adev->boot_completed=get_boot_status();
        if(adev->boot_completed==true){
            //boot completed
            do_bootcomplated(dev);
        }
    }
}
...
static void create_bootup_timer(struct tiny_audio_device * adev,int delay){
    ...
    if(status == 0){
        ...
        ret =timer_settime(adev->bootup_timer.timer_id, 0, &ts, 0);
        if( ret ){
            ALOGW("timer_settime failed");
            adev->boot_completed=true;
            timer_delete(adev->bootup_timer.timer_id);
            adev->bootup_timer.created = false;
        }
...
bool i2s_switch_sel(struct tiny_audio_device *adev, cp_type_t type)
{
...
#ifdef NXP_SMART_PA
    struct route_setting *i2s_route_smt = NULL;
#endif
    struct route_setting *i2s_loop = NULL;

    i2s_loop = modem->i2s_fm_bt_loop;
...
#ifdef NXP_SMART_PA
        case EXT_CODEC_AP:
            i2s_route_smt = modem->i2s_switch_ext_codec_ap;
            break;
        case EXT_CODEC_CP:
            i2s_route_smt = modem->i2s_switch_ext_codec_cp;
            break;
#endif
        default:
    ...
    if (NULL != i2s_loop) {
            struct mixer_ctl *ctl = mixer_get_ctl_by_name(mixer, i2s_loop->ctl_name);
            if (NULL != ctl) {
                if (i2s_loop->strval) {
                    ret = mixer_ctl_set_enum_by_string(ctl, i2s_loop->strval);
                    if (ret != 0) {
                        ALOGE("Failed to set '%s' to '%s'\n",
                                i2s_loop->ctl_name, i2s_loop->strval);
                    } else {
                        ALOGI("Set '%s' to '%s'\n",
                                i2s_loop->ctl_name, i2s_loop->strval);
                    }
                    ...
                    /* This ensures multiple (i.e. stereo) values are set jointly */
                    for (j=0; j < mixer_ctl_get_num_values(ctl); j++) {
                        ret = mixer_ctl_set_value(ctl, j, i2s_loop->intval);
                        if (ret != 0) {
                            ALOGE("Failed to set '%s'.%d to %d\n",
                                    i2s_loop->ctl_name, j, i2s_loop->intval);
                        } else {
                            ALOGV("Set '%s'.%d to %d\n",
                                    i2s_loop->ctl_name, j, i2s_loop->intval);
                        }
...
#ifdef NXP_SMART_PA
    if (NULL != i2s_route_smt) {
            struct mixer_ctl *ctl = mixer_get_ctl_by_name(mixer, i2s_route_smt->ctl_name);
            if (NULL != ctl) {
                if (i2s_route_smt->strval) {
                    ret = mixer_ctl_set_enum_by_string(ctl, i2s_route_smt->strval);
                    if (ret != 0) {
                        ALOGE("Failed to set '%s' to '%s'\n",
                                i2s_route_smt->ctl_name, i2s_route_smt->strval);
                    } else {
                        ALOGI("Set '%s' to '%s'\n",
                                i2s_route_smt->ctl_name, i2s_route_smt->strval);
                    }
                    ...
                    /* This ensures multiple (i.e. stereo) values are set jointly */
                    for (j=0; j < mixer_ctl_get_num_values(ctl); j++) {
                        ret = mixer_ctl_set_value(ctl, j, i2s_route_smt->intval);
                        if (ret != 0) {
                            ALOGE("Failed to set '%s'.%d to %d\n",
                                    i2s_route_smt->ctl_name, j, i2s_route_smt->intval);
                        } else {
                            ALOGV("Set '%s'.%d to %d\n",
                                    i2s_route_smt->ctl_name, j, i2s_route_smt->intval);
                        }
        ...
        int on)
{
    ...
    ALOGI("[get_route_setting], warning: devices(0x%08x) NOT found.", devices);
    return 0;
}
...
static void do_select_devices_static(struct tiny_audio_device *adev)
{
    ...
    int ctl_on_count=0;
#endif
    int set_fm_speaker=adev->set_fm_speaker;
...
ret);
    ...
    if((adev->fm_type != AUD_FM_TYPE_LINEIN_3GAINS) && adev->fm_open) {
        ALOGI("static before fm_mute_l :%d,cur_out:%x,adev->set_fm_speaker:%d",adev->fm_mute_l,cur_out,adev->set_fm_speaker);
        if(false==adev->headset_connected){
            set_fm_speaker=1;
            ALOGI("headset disconnect,set_fm_speaker ");
        }else{
            set_fm_speaker=adev->set_fm_speaker;
        }

        if(((set_fm_speaker) && (!(cur_out & AUDIO_DEVICE_OUT_SPEAKER)))
            || ((!set_fm_speaker) && (!((cur_out & AUDIO_DEVICE_OUT_WIRED_HEADSET) || (cur_out & AUDIO_DEVICE_OUT_WIRED_HEADPHONE))))) {
            ...
            adev->fm_mute_l = true;
            ALOGI("static do fm_mute_l  :%d,cur_out:%x,adev->set_fm_speaker:%d",adev->fm_mute_l,cur_out,adev->set_fm_speaker);
        }
    }
	...
	/* separate INPUT/OUTPUT case for some common bit used. */
        ...
        if (AUDIO_DEVICE_OUT_FM == adev->dev_cfgs[i].mask && adev->pcm_fm_dl == NULL) {
            ALOGI("%s:open FM device",__func__);
    ...
    for (i = 0; i < adev->num_dev_cfgs; i++) {
        ...
        if(AUDIO_DEVICE_OUT_FM == adev->dev_cfgs[i].mask && adev->pcm_fm_dl != NULL)
        {
            ALOGI("%s:close FM device",__func__);
...
static int out_device_disable(struct tiny_audio_device *adev,int out_dev)
{
    ...
    for (i = 0; i < adev->num_dev_cfgs; i++) {
        if ((out_dev & adev->dev_cfgs[i].mask)
            ...
            adev->dev_cfgs[i].off_len);
            if(AUDIO_DEVICE_OUT_FM == adev->dev_cfgs[i].mask && adev->pcm_fm_dl != NULL) {
                ALOGW("%s:close FM device",__func__);
                pcm_close(adev->pcm_fm_dl);
                adev->pcm_fm_dl= NULL;
            }
        }
   ...
   pthread_mutex_unlock(&adev->lock);
    ...
    if(cur_out && (!(cur_out & (~AUDIO_DEVICE_OUT_ALL_SCO)))) {
        if((adev->active_output || adev->active_deepbuffer_output )
            ...
            ALOGI("yaye  need_bypass_output!%s, adev->active_output:%p,  adev->active_deepbuffer_output:%p, bt_sco_state:%x",
    ...
    if(cur_in && (!(cur_in & (~AUDIO_DEVICE_IN_ALL_SCO)))) {
        if((adev->active_input)
            ...
            ALOGI("yaye  need_bypass_input!adev->active_input:%p, bt_sco_state:%x",
    ...
    if((adev->fm_type != AUD_FM_TYPE_LINEIN_3GAINS) && adev->fm_open
        && adev->pcm_fm_dl) {
        ALOGI("before fm_mute_l :%d,cur_out:%x,adev->set_fm_speaker:%d,adev->fm_pre_stop:%d",adev->fm_mute_l,cur_out,adev->set_fm_speaker,adev->fm_pre_stop);
        if(adev->fm_pre_stop || ((set_fm_speaker) && (!(cur_out & AUDIO_DEVICE_OUT_SPEAKER)))
            || ((!set_fm_speaker) && (!((cur_out & AUDIO_DEVICE_OUT_WIRED_HEADSET) || (cur_out & AUDIO_DEVICE_OUT_WIRED_HEADPHONE))))) {
                if (adev->private_ctl.fm_da0_mux)
                    mixer_ctl_set_value(adev->private_ctl.fm_da0_mux, 0, 0);
                if (adev->private_ctl.fm_da1_mux)
                ...
                adev->fm_mute_l = true;
                fm_need_mute_l = true;
            ALOGI("do fm_mute_l  :%d,cur_out:%x,adev->set_fm_speaker:%d",adev->fm_mute_l,cur_out,adev->set_fm_speaker);
        }
    }

    pthread_mutex_lock(&adev->mute_lock);
    if(adev->cache_mute == false && ((pre_out & AUDIO_DEVICE_OUT_SPEAKER)&& (!(pre_out & (AUDIO_DEVICE_OUT_WIRED_HEADPHONE|AUDIO_DEVICE_OUT_WIRED_HEADSET))))
        && ((cur_out & (AUDIO_DEVICE_OUT_WIRED_HEADPHONE|AUDIO_DEVICE_OUT_WIRED_HEADSET))&& (!(cur_out & AUDIO_DEVICE_OUT_SPEAKER)))) {
        do_speaker_mute(adev, true);
        speaker_mute = true;
    }
    pthread_mutex_unlock(&adev->mute_lock);
...
#endif
                fm_iis_connection = true;
                ALOGI("do_select_devices, fm_volume: %d",adev->fm_volume);
                pthread_mutex_lock(&adev->lock);
                if(adev->fm_volume !=0){
                    if (pre_out != cur_out) {
                        if ((false == adev->force_fm_mute) &&
                                (false == adev->fm_mute_apk)) {
                            set_fm_mute_noramp(adev, false);
                        } else {
...
out:
    ...
    if((adev->fm_type != AUD_FM_TYPE_LINEIN_3GAINS) && adev->fm_open
        && adev->pcm_fm_dl) {
        ALOGI("before fm unmute_l :%d,cur_out:%x,adev->set_fm_speaker:%d,adev->fm_pre_stop:%d,adev->fm_mute:%d, adev->fm_volume:%d",adev->fm_mute_l,cur_out,adev->set_fm_speaker,adev->fm_pre_stop,
            adev->fm_mute, adev->fm_volume);
        if(!(adev->fm_pre_stop || ((set_fm_speaker) && (!(cur_out & AUDIO_DEVICE_OUT_SPEAKER)))
            || ((!set_fm_speaker) && (!((cur_out & AUDIO_DEVICE_OUT_WIRED_HEADSET) || (cur_out & AUDIO_DEVICE_OUT_WIRED_HEADPHONE)))))) {
            if(adev->fm_mute_l == true){
                adev->fm_mute_l = false;
                if((adev->fm_mute == false) &&  adev->fm_volume){
                    if (adev->private_ctl.fm_da0_mux)
                        mixer_ctl_set_value(adev->private_ctl.fm_da0_mux, 0, 1);
                    if(adev->private_ctl.fm_da1_mux)
                        mixer_ctl_set_value(adev->private_ctl.fm_da1_mux, 0, 1);
                    ALOGI("do fm unmute :%d,cur_out:%x,adev->set_fm_speaker:%d",adev->fm_mute_l,cur_out,adev->set_fm_speaker);
                }
            }
        }
    }
    ...
    if(speaker_mute) {
        pthread_mutex_lock(&adev->mute_lock);
        if(adev->cache_mute == false){
            do_speaker_mute(adev, false);
            speaker_mute = false;
        }
        pthread_mutex_unlock(&adev->mute_lock);
    }
...
static void select_devices_signal(struct tiny_audio_device *adev)
{
    ALOGI("select_devices_signal starting... adev->out_devices 0x%x adev->in_devices 0x%x",adev->out_devices,adev->in_devices);
...
static void select_devices_signal_asyn(struct tiny_audio_device *adev)
{
    ALOGI("select_devices_signal_asyn starting... adev->out_devices 0x%x adev->in_devices 0x%x",adev->out_devices,adev->in_devices);
    sem_post(&adev->routing_mgr.device_switch_sem);
...
void force_all_standby(struct tiny_audio_device *adev)
{
    ...
    if (adev->active_fmul_input) {
        in = adev->active_fmul_input;
        pthread_mutex_lock(&in->lock);
        do_input_standby(in);
        pthread_mutex_unlock(&in->lock);
    }
        ...
        size_t bytes)
{
    struct tiny_stream_in *in = (struct tiny_stream_in *)stream;
        ...
        pthread_mutex_unlock(&in->lock);
        ALOGI("hidl_in_read:%d ret:%d",bytes,ret);
        return bytes;
}
#endif
...
static void select_mode(struct tiny_audio_device *adev)
{
    if (adev->mode == AUDIO_MODE_IN_CALL) {
        ALOGI("Entering IN_CALL state, %s first call...out_devices:0x%x mode:%d ", adev->call_start ? "not":"is",adev->out_devices,adev->mode);
    } else {
        ALOGI("Leaving IN_CALL state, call_start=%d, mode=%d out_devices:0x%x ",
...
static int start_vaudio_output_stream(struct tiny_stream_out *out)
{
    ...
    if((out->flags & AUDIO_OUTPUT_FLAG_DEEP_BUFFER)||(out->audio_app_type == AUDIO_HW_APP_VOICE_TX))
        port = 1;
    else
        port = 0;

    ALOGI("start vaudio port :%d,flags:%d,%d,%d,out->audio_app_type:%d",port,(int)out->flags,(int)(out->flags & AUDIO_OUTPUT_FLAG_DEEP_BUFFER),(int)((out->flags & AUDIO_OUTPUT_FLAG_DEEP_BUFFER)||(out->audio_app_type == AUDIO_HW_APP_VOICE_TX)),out->audio_app_type);
    ...
    if(out->audio_app_type == AUDIO_HW_APP_VOICE_TX) {
        adev->active_voicetx_output = out;
    }
    return 0;
...
static int start_sco_output_stream(struct tiny_stream_out *out)
{
    ...
    ALOGI("start_sco_output_stream ok");
    return 0;
...
static int start_bt_sco_output_stream(struct tiny_stream_out *out)
{
    ...
    */
    ALOGI("bt sco : %s before", __func__);
    ...
    ALOGI("start_bt_sco_output_stream error ok");
    return 0;
...
static int start_output_stream(struct tiny_stream_out *out)
{
    ...
    unsigned int port = PORT_MM;
    int ret=0;
    aud_misc_ctl_enable_pownerhint(adev->misc_ctl);
    ...
    ALOGD("start output stream mode:%x devices:%x call_start:%d, call_connected:%d, is_voip:%d, voip_state:%x, is_bt_sco:%d,out->audio_app_type:%d",
    ...
    if (!adev->call_start && adev->voip_state == 0) {
        ...
        if(out->devices != AUDIO_DEVICE_OUT_TELEPHONY_TX) {
    ...
    if(out->is_voip_mixer){
        ...
        if(out->audio_app_type == AUDIO_HW_APP_VOICE_TX) {
            set_cp_mixer_type(adev->cp_mixer[adev->cp_type], MIXER_TO_VOICE_UPLINK_ENABLE);
        }
        else {
        ...
        ALOGI("start_output_stream open s_tinycard in");
        card = s_tinycard;
        ...
        if (out->flags & AUDIO_OUTPUT_FLAG_DEEP_BUFFER) {
            ...
            if (out->low_power) {
                    ...
                    ALOGI("out->write_threshold=%d, config.avail_min=%d %dms, start_threshold=%d",
        ...
        ALOGI("start_output_stream open s_tinycard successfully");
    }
...
static size_t get_input_buffer_size(uint32_t sample_rate, int format, int channel_count)
{
    ...
    ALOGD("%s line:%d limit size:%zd sample_rate:%d channel_count:%d",
        __func__,__LINE__,size,sample_rate,channel_count);
    return size;
}
...
static int do_output_standby(struct tiny_stream_out *out)
{
    ...
    if (!out->standby) {
        ...
        if(out->audio_app_type == AUDIO_HW_APP_VOICE_TX) {
            set_cp_mixer_type(adev->cp_mixer[adev->cp_type], MIXER_TO_VOICE_UPLINK_DISABLE);
            adev->active_voicetx_output = NULL;
        }
	...
	mux_pcm_close(out->pcm_voip);
        ...
        if(out->pcm_bt_sco) {
            ...
            */
            ...
            ALOGI("bt sco : %s downlink is not exist", __func__);
            adev->bt_sco_state &= (~BT_SCO_DOWNLINK_IS_EXIST);
        }
        if(out->buffer_bt_sco) {
            free(out->buffer_bt_sco);
    ...
    aud_misc_ctl_disable_pownerhint(out->dev->misc_ctl);
    ...
    struct tiny_audio_device *adev = out->dev;
     if (out->audio_app_type == AUDIO_HW_APP_OFFLOAD) {
        ...
        if (out->compress != NULL) {
            ALOGI("audio_offload compress_close");
...
#ifdef NXP_SMART_PA
void do_output_standby_nxptfa(struct tiny_stream_out *out)
{
    struct tiny_audio_device *adev = out->dev;

    ALOGI("%s audio_app_type %d, smart_pa_exist %d, nxp_pa %d", __func__,
        out->audio_app_type, adev->smart_pa_exist, out->nxp_pa);
...
void FMSpkCheckAndClose(struct tiny_audio_device *adev,int val)
{
    if(!(val&AUDIO_DEVICE_OUT_SPEAKER)) {
        adev->fm_spk_bypass=1;
        if(adev->active_fmul_input) {
            do_input_standby(adev->active_fmul_input);
        }
        if(adev->active_input && adev->active_input->is_fm_rec) {
            do_input_standby(adev->active_input);
        }
    }
    else {
        adev->fm_spk_bypass=0;
    }
}

#ifdef AUDIOHAL_V4
static int adev_check_out_devices(void *dev,audio_devices_t device){
    struct tiny_stream_out *out = NULL;
    struct tiny_audio_device *adev = (struct tiny_audio_device *)dev;
    struct listnode *item;
    struct listnode *item2;
    ...
    if(!list_empty(&adev->active_out_list)){
        list_for_each_safe(item, item2,&adev->active_out_list){
            ...
            if(out!=NULL){
                pthread_mutex_lock(&out->lock);
                ALOGI("adev_check_out_devices %p %d %d",out,out->devices,device);
                out->devices=device;
...
static int adev_check_input_devices(void *dev,audio_devices_t device){
...
static int out_set_parameters(struct audio_stream *stream, const char *kvpairs)
{
    ...
    ALOGI("[out_set_parameters], kvpairs=%s devices:0x%x mode:%d ", kvpairs,adev->out_devices,adev->mode);
    ...
    if (ret >= 0) {
        ...
        pthread_mutex_lock(&adev->lock);
#ifdef NXP_SMART_PA_HIDL
        FMSpkCheckAndClose(adev,val);
#endif
        if(val!=0){
            pthread_mutex_lock(&out->lock);
            out->apm_devices=val;
        ...
        if(adev->boot_completed==false){
            if(((AUDIO_DEVICE_OUT_WIRED_HEADPHONE|AUDIO_DEVICE_OUT_WIRED_HEADSET)&val)
                ...
                ALOGI("out_set_parameters connect devices, boot completed");
                adev->boot_completed=true;
            }else{
                check_boot_status(adev);
                if(adev->boot_completed==false){
                    val=0;
                    ALOGI("system is not ready, can not set audio devices form audiopolicymanager");
                }
            }
        }
        pthread_mutex_lock(&out->lock);
		  if (((adev->out_devices & AUDIO_DEVICE_OUT_ALL) != val) && ((val != 0)) //val=0 will cause XRUN. So ignore the "val=0"expect for closing FM path.
            ...
            out->devices = val;
            ALOGI("out_set_parameters want to set devices:0x%x old_mode:%d new_mode:%d call_start:%d ",adev->out_devices,cur_mode,adev->mode,adev->call_start);
            ...
            if((!adev->call_start)&&(!adev->voip_state))
                select_devices_signal(adev);
            if((AUDIO_MODE_IN_CALL == adev->mode) || adev->voip_start || adev->call_start) {
                ret = at_cmd_route(adev);  //send at command to cp
                ALOGI("at_cmd_route:ret:%d", ret);
            }
            pthread_mutex_unlock(&out->lock);
            pthread_mutex_unlock(&adev->lock);
          } else {
            if(val != 0) {
              ...
              pthread_mutex_unlock(&out->lock);
            pthread_mutex_unlock(&adev->lock);
            ALOGI("the same devices(0x%x) with val(0x%x) val is zero...",adev->out_devices,val);
        }
        ret = 0 ;
    }
    ...
    if(AUDIO_HW_APP_HIDL_OUTPUT==out->audio_app_type){
        ...
        if (ret >= 0) {
            ...
            pthread_mutex_unlock(&adev->lock);
            ALOGI("sprdhidl_writesize:%d",val);
        }
    }
#endif
...
static char * out_get_parameters(const struct audio_stream *stream, const char *keys)
{
    ...
    ALOGI("out_get_parameters;%s",keys);
...
static bool out_bypass_data(struct tiny_stream_out *out, uint32_t frame_size, uint32_t sample_rate, size_t bytes)
{
    ...
    if (adev->loopback_start) {
        ALOGI("%s, out bypass loopback mixer", __func__);
        pthread_mutex_unlock(&adev->lock);
        pthread_mutex_unlock(&out->lock);
        usleep((int64_t) bytes * 1000000 / frame_size / sample_rate);
        return true;
    ...
    if (0
            ...
            || ((!adev->call_connected)&&(out->audio_app_type == AUDIO_HW_APP_VOICE_TX))) {
        LOG_V("out_write throw away data call_start(%d) mode(%d) devices(0x%x) call_connected(%d) vbc_2arm(%d) call_prestop(%d),out->audio_app_type:%d",
            adev->call_start,adev->mode,adev->out_devices,
            adev->call_connected,adev->vbc_2arm,adev->call_prestop,out->audio_app_type);
#ifdef AUDIO_DEBUG
            if((TEST_AUDIO_IN_OUT_LOOP==adev->ext_contrl->dev_test.test_mode)
                ||(TEST_AUDIO_CP_LOOP==adev->ext_contrl->dev_test.test_mode)){
                ALOGW("out_bypass_data TEST_AUDIO_IN_OUT_LOOP");
            }
#endif
        pthread_mutex_unlock(&adev->lock);
        pthread_mutex_unlock(&out->lock);
        ...
        size_t bytes)
{
    ...
    LOG_V("out_write_vaudio in");
    if(out->pcm_vplayback) {
        LOG_V("out_write_vaudio in bytes is %d",bytes);
        ...
        size_t bytes)
{
    ...
    in_frames = bytes / frame_size;
    out_frames = RESAMPLER_BUFFER_SIZE / frame_size;
    LOG_V("out_write_sco in bytes is %d,frame_size %d, in_frames %d, out_frames %d,out->pcm_voip %x", bytes, frame_size,in_frames, out_frames,out->pcm_voip);
    if(out->pcm_voip) {
        out->resampler_sco->resample_from_input(out->resampler_sco,
                (int16_t *)buffer,
                &in_frames,
    ...
    LOG_V("voip:out_write_sco out bytes is %d,frame_size %d, in_frames %d, out_frames %d,out->pcm_voip %x", bytes, frame_size,in_frames, out_frames,out->pcm_voip);
    return ret;
}
        ...
        size_t bytes)
{
    ...
    in_frames = bytes / frame_size;
    out_frames = RESAMPLER_BUFFER_SIZE / frame_size;
    LOG_V("out_write_bt_sco in bytes is %d,frame_size %d, in_frames %d, out_frames %d,out->pcm_voip %x", bytes, frame_size,in_frames, out_frames,out->pcm_voip);
    if(out->pcm_bt_sco) {
       ...
       if(adev->ext_contrl->dump_info->dump_bt_sco){
        ...
        LOG_V("out_write_bt_sco  bt_type:%d, out_frames:%d, out_byte:%d", adev->bluetooth_type, out_frames, out_bytes);
    ...
    LOG_V("voip:out_write_bt_sco out bytes is %d,frame_size %d, in_frames %d, out_frames %d,out->pcm_voip %x", bytes, frame_size,in_frames, out_frames,out->pcm_voip);
    return ret;
}
        ...
        size_t bytes)
{
     ...
     * mutex
     */
    LOG_V("out_write1: out->flags %d out->devices %x,start: out->is_voip is %d, out->is_voip_mixer is %d adev->voip_state is %d,adev->voip_start is %d,out->audio_app_type:%d",
                out->flags, out->devices,out->is_voip,out->is_voip_mixer,adev->voip_state,adev->voip_start,out->audio_app_type);
    pthread_mutex_lock(&adev->lock);
    pthread_mutex_lock(&out->lock);
    ...
    if((!(out->is_voip |out->is_voip_mixer)) && adev->voip_state &&(out->audio_app_type != AUDIO_HW_APP_OFFLOAD)) {
        ALOGW("out_write: drop data and sleep,out->is_voip is %d, adev->voip_state is %d,adev->voip_start is %d",out->is_voip,adev->voip_state,adev->voip_start);
        pthread_mutex_unlock(&out->lock);
        pthread_mutex_unlock(&adev->lock);
        usleep(100000);
        return bytes;
        ...
        if((!(out->is_bt_sco||out->is_voip || adev->call_start || (out->audio_app_type == AUDIO_HW_APP_OFFLOAD)))
            ...
            if(!out->nxp_pa) {
                ...
                if(out->handle_pa) {
                    ...
                    adev->active_output = out;
                    register_nxp_pcmdump(out->handle_pa,audio_pcm_dump);
                }
            }
        } else {
        ...
        if(out->flags & AUDIO_OUTPUT_FLAG_DEEP_BUFFER){
            ...
            if ((low_power != out->low_power) /*&& (out->flags & AUDIO_OUTPUT_FLAG_DEEP_BUFFER)*/) {
                if (low_power) {
                    ...
                    ALOGI("low_power out->write_threshold=%d, config.avail_min=%d, start_threshold=%d",
                            out->write_threshold,out->config.avail_min, out->config.start_threshold);
                } else {
                    out->config.avail_min = SHORT_PERIOD_SIZE;
                    ALOGI("out->write_threshold=%d, config.avail_min=%d, start_threshold=%d",
                            out->write_threshold,out->config.avail_min, out->config.start_threshold);
                }
                pcm_set_avail_min(out->pcm, out->config.avail_min);
                out->low_power = low_power;
...
static void *audio_bt_sco_dup_thread_func(void * param)
{
    ...
    prctl(PR_SET_NAME, (unsigned long)"Audio Bt Sco Thread", 0, 0, 0);
    ...
    while(!adev->bt_sco_manager.thread_is_exit) {
        if(adev->bt_sco_manager.dup_need_start) {
            ...
            if(bt_sco_playback == NULL) {
                ALOGI("bt sco : duplicate downlink card opening");
                if(adev->bluetooth_type == VX_WB_SAMPLING_RATE)
                ...
                ALOGI("bt sco : duplicate open downlink card signal");
            ...
            if(bt_sco_playback) {
                ...
                ALOGI("bt sco : duplicate close downlink card signal");
                sem_post(&adev->bt_sco_manager.dup_sem);
            }
            ALOGI("bt sco : duplicate thread wait for start");
            pthread_cond_wait(&adev->bt_sco_manager.cond, &adev->bt_sco_manager.cond_mutex);
            ALOGI("bt sco : duplicate thread is started now ...");
        }
    }
    pthread_mutex_unlock(&adev->bt_sco_manager.cond_mutex);
...
{
    int ret = 0;
    ALOGI("bt sco : %s duplicate thread %s", __func__, (enable ? "start": "stop"));
    ...
    if(enable != adev->bt_sco_manager.dup_count) {
        ...
        ALOGI("bt sco : %s %s before wait", __func__, (enable ? "start": "stop"));
        sem_wait(&adev->bt_sco_manager.dup_sem);
        ALOGI("bt sco : %s %s after wait", __func__, (enable ? "start": "stop"));
    }
...
static void audio_bt_sco_thread_destory(struct tiny_audio_device *adev)
{
    ...
    ALOGI("bt sco : duplicate thread destory before");
    pthread_cond_signal(&adev->bt_sco_manager.cond);
    pthread_mutex_unlock(&adev->bt_sco_manager.cond_mutex);
...
static int check_support_nr(struct tiny_audio_device *adev,int rate){
    ...
    NR_CONTROL_PARAM_T *pNvInfo_handsfree = (NR_CONTROL_PARAM_T *)
        (&(audio_para_ptr[GetAudio_InMode_number_from_device(adev, AUDIO_DEVICE_IN_BUILTIN_MIC)].audio_enha_eq.nrArray[0]));
    NR_CONTROL_PARAM_T *pNvInfo_headset = (NR_CONTROL_PARAM_T *)
        (&(audio_para_ptr[GetAudio_InMode_number_from_device(adev, AUDIO_DEVICE_IN_WIRED_HEADSET)].audio_enha_eq.nrArray[0]));
    ...
    if(false==adev->forceuse_nr){
        ALOGI("check_support_nr fource un-use nr");
        return false;
    }
...
static int start_input_stream(struct tiny_stream_in *in)
{
    ...
    ALOGW("start_input_stream in mode:%x devices:%x call_start:%d, is_voip:%d, is_bt_sco:%d, adev->fm_open %d adev->fm_record %d,is_fm_rec:%d,fm_uldl:%d,in->device:%x,active input:%zu, in->source:%d",adev->mode,adev->in_devices,adev->call_start,
            in->is_voip, in->is_bt_sco, adev->fm_open , adev->fm_record,in->is_fm_rec,adev->fm_uldl,in->device,(size_t)adev->active_input,in->source);
    if (!adev->call_start) {
	...
	in->mux_pcm = mux_pcm_open(SND_CARD_VOIP_TG,PORT_MM,PCM_IN,&in->config );
	 if (!pcm_is_ready(in->mux_pcm)) {
...
#else
        in->pcm = pcm_open(s_voip,PORT_MM,PCM_IN|PCM_MONOTONIC,&in->config );
        in->is_mmap = false;
        ...
        if(!adev->aud_proc_init) {
            ...
            in->active_rec_proc = init_rec_process(GetAudio_InMode_number_from_device(adev, adev->in_devices), in->requested_rate );
        ...
        if(in->requested_rate != in->config.rate) {
            ALOGI("bt sco input : in->requested_rate is %d, in->config.rate is %d",in->requested_rate, in->config.rate);
            ret= in_init_resampler(in);
            ALOGI("bt sco input : in_init_resampler ret is %d",ret);
            if(ret){
                goto err;
            }
        }
...
#else
        in->pcm = pcm_open(card,PORT_MM,PCM_IN|PCM_MONOTONIC,&in->config);
        in->is_mmap = false;
        if (!pcm_is_ready(in->pcm)) {
...
#else
        if(adev->fm_open &&( adev->fm_record ||in->is_fm_ul)) {
#endif
            if(adev->fm_record && !in->is_fm_ul){
                in->is_fm_rec = true;
            }
...
#ifdef RECORD_USE_MMAP
                   in->pcm = pcm_open(s_tinycard, PORT_MM, PCM_IN|PCM_MMAP|PCM_NOIRQ|PCM_MONOTONIC, &in->config);
                   in->is_mmap = true;
#else
                   in->pcm = pcm_open(s_tinycard, PORT_MM, PCM_IN|PCM_MONOTONIC, &in->config);
                   in->is_mmap = false;
#endif
                   if(!pcm_is_ready(in->pcm)) {
                     dump_all_audio_reg(-1,adev->dump_flag);
          ...
          if(in->config.channels != in->requested_channels) {
            ...
            in->flag_i2s_ad0_set = true;
#ifdef RECORD_USE_MMAP
            in->pcm = pcm_open(s_tinycard, PORT_MM_C, PCM_IN|PCM_MMAP|PCM_NOIRQ|PCM_MONOTONIC, &in->config);
            in->is_mmap = true;
#else
            in->pcm = pcm_open(s_tinycard, PORT_MM_C, PCM_IN|PCM_MONOTONIC, &in->config);
            in->is_mmap = false;
#endif
            if(!pcm_is_ready(in->pcm)) {
            ...
            if((is_support_nr==true) &&(in->config.rate != in->requested_rate)&& (!adev->aud_proc_init) ){
                int16 *pNvInfo = &(audio_para_ptr[GetAudio_InMode_number_from_device(adev, adev->in_devices)].audio_enha_eq.nrArray[0]);
                if (NULL == pNvInfo) {
    ...
    if((in->requested_rate != in->config.rate) && (NULL == in->Tras48To44)) {
        ALOGI(": in->requested_rate is %d, in->config.rate is %d",in->requested_rate, in->config.rate);
        ret= in_init_resampler(in);
    ...
    ALOGI("start_input,channels=%d,peroid_size=%d, peroid_count=%d,rate=%d",
    ...
    if(in->is_fm_ul) {
        adev->active_fmul_input = in;
    }
    else {
        adev->active_input = in;
    }

    ALOGI("start input stream out");
    return 0;
...
static int in_get_active_microphones(const struct audio_stream_in *stream,
    ...
    pthread_mutex_lock(&adev->lock);
    pthread_mutex_lock(&in->lock);
    int ret = platform_get_active_microphones(adev->audio_data, in->device,
                                              in->config.channels,
...
static int do_input_standby(struct tiny_stream_in *in)
{
    ...
    if (!in->standby) {
        ALOGV("%s, standby=%d, in_devices=0x%08x,in->is_fm_rec:%d,in->is_fm_ul:%d", __func__, in->standby, adev->in_devices,in->is_fm_rec,in->is_fm_ul);
#ifdef AUDIO_MUX_PCM
        if (in->mux_pcm) {
            mux_pcm_close(in->mux_pcm);
            in->mux_pcm = NULL;
...
#ifdef NXP_SMART_PA
        if(adev->smart_pa_exist && in->FmRecHandle) {
            ALOGI("fm_pcm_close in do_input_standby");
            fm_pcm_close(in->FmRecHandle);
            in->FmRecHandle = NULL;
        }
#endif
        ...
        pthread_mutex_lock(&adev->input_lock);
        adev->active_input = 0;
        pthread_mutex_unlock(&adev->input_lock);
...
static int in_set_parameters(struct audio_stream *stream, const char *kvpairs)
{
    ...
    if (ret >= 0) {
        val = atoi(value);
        ALOGI("AUDIO_PARAMETER_STREAM_INPUT_SOURCE in_set_parameters val %d",val);
    ...
    if (ret >= 0) {
        ...
        if(ret >= 0){
            ...
            if(val){
                 ...
                 adev->test_ctl.input_test.hidl_inputstream=in;
                 adev->test_ctl.loop_test.hidl_inputstream=in;
             }
        }
        ...
        const char *keys)
{
     ...
     ALOGI("in_get_parameters;%s",keys);
        ...
        struct resampler_buffer* buffer)
{
    ...
    if (in->frames_in == 0) {
        if(in->mux_pcm){
            ...
            if(in->is_mmap){
                in->read_status = pcm_mmap_read(in->pcm,
                        (void*)in->buffer,
                        in->config.period_size *
                        audio_stream_frame_size((const struct audio_stream *)(&in->stream.common)));
            }
        ...
        if((in->config.channels == 2) && (PCM_FORMAT_S16_LE==in->config.format)
            &&((in->device==AUDIO_DEVICE_IN_BUILTIN_MIC)
            ||(in->device==AUDIO_DEVICE_IN_BACK_MIC))){
            left2right(in->buffer,in->config.period_size * audio_stream_frame_size(&in->stream.common)/2);
        }
#ifdef AUDIO_DEBUG
...
static bool in_bypass_data(struct tiny_stream_in *in,uint32_t frame_size, uint32_t sample_rate, void* buffer, size_t bytes)
{
    ...
    /*
       ...
       */
        ...
        if((TEST_AUDIO_IN_OUT_LOOP==adev->ext_contrl->dev_test.test_mode)
            ...
            ALOGI("in_bypass_data TEST_AUDIO_IN_OUT_LOOP");
            pthread_mutex_unlock(&adev->lock);
            pthread_mutex_unlock(&in->lock);
            usleep((int64_t)bytes * 1000000 / frame_size / sample_rate);
            return true;
    ...
    if ((!adev->call_start) && ((in->device == AUDIO_DEVICE_IN_VOICE_CALL))
        || adev->call_prestop ||((in->source == AUDIO_SOURCE_VOICE_RECOGNITION) &&  adev->fm_open && adev->record_bypass && !adev->fm_record)
        ||(in->is_fm_rec &&((adev->active_input && (!adev->active_input->is_fm_rec)) || adev->fm_spk_bypass))
        || (in->is_fm_ul && ((adev->active_input && (!adev->active_input->is_fm_rec)) || adev->fm_spk_bypass))
        || (in->is_fm_ul && adev->call_start)
        || (in->is_fm_ul && adev->voip_state)){
        ...
        size_t bytes)
{
...
#ifdef AUDIO_VOIP_VERSION_2
    if((adev->voip_playback_start && (in->source != AUDIO_SOURCE_VOICE_COMMUNICATION ||
    ...
    if(in->source_to_set != in->source) {
        in->source = in->source_to_set;
        do_input_standby(in);
    }
...
#ifdef NXP_SMART_PA_HIDL
    if((!adev->call_start)&& (!in->is_voip)&& (adev->fm_open || adev->active_fmul_input) && (!in->is_fm_ul)) {
        if(!adev->fm_record) {
            ALOGE("standy fm adev->fm_record:%d,adev->active_fmul_input:%zu",adev->fm_record,(size_t)adev->active_fmul_input);
            if(adev->active_input && adev->active_input->is_fm_rec) {
                do_input_standby(adev->active_input);
            }
            if(adev->active_fmul_input) {
...
#else
    if((!adev->call_start)&& (!in->is_voip) && (!in->is_bt_sco) && adev->fm_open ) {
        if(!adev->fm_record) {
            ALOGE("standy fm adev->fm_record:%d,adev->active_fmul_input:%zu",adev->fm_record,(size_t)adev->active_fmul_input);
            if(adev->active_input && adev->active_input->is_fm_rec) {
                do_input_standby(adev->active_input);
    ...
    if((adev->fm_record)
        ...
        ||(in->is_voip) || (in->is_fm_ul) || (in->is_fm_rec)){
    ...
    if (in->flag_i2s_ad0_set == true) {
        vbc_ad01iis_select(adev, false);
        in->flag_i2s_ad0_set = false;
    }
    ...
    if(in->pop_mute) {
        if(in->is_voip) {
            ALOGI("is voip no need to mute at start");
            in->pop_mute = false;
        }
            ...
            // mute 240ms for pop
            ALOGI("set mute in_read bytes %d in->pop_mute_bytes %d",bytes,in->pop_mute_bytes);
...
#if (defined AUDIO_HAL_ANDROID_N_API) || (defined AUDIO_HAL_ANDROID_Q_API)
                                   ...
                                   int64_t *frames, int64_t *time)
{
    ...
    if (in->pcm) {
        ...
        if (pcm_get_htimestamp(in->pcm, &avail, &timestamp) == 0) {
            if(in->requested_rate != in->config.rate && in->config.rate != 0)
                avail = (avail * in->requested_rate)/in->config.rate;

            *frames = in->frames_read + avail;
            *time = timestamp.tv_sec * 1000000000LL + timestamp.tv_nsec;
            ret = 0;
            LOG_V("position:%lld %lld",*time,*frames);
        }
    }
    pthread_mutex_unlock(&in->lock);
    return ret;
...
int audio_del_input(struct tiny_audio_device *adev,struct tiny_stream_in *in)
{
    struct listnode *item;
    struct listnode *item2;
    struct tiny_stream_in * in_tmp = NULL;
        ...
        struct audio_stream_out **stream_out)
{
    ...
    ALOGI("adev_open_output_stream, devices=%d", devices);
    ...
    if(ladev->boot_completed==false){
        if(devices&(AUDIO_DEVICE_OUT_WIRED_HEADSET|AUDIO_DEVICE_OUT_WIRED_HEADPHONE)){
            ALOGI("adev_open_output_stream headset connected");
            ladev->boot_completed=true;
        }else{
            devices=get_headset_device(ladev);
            if(devices){
                out->devices=devices;
                ALOGI("adev_open_output_stream use hal devices:0x%x",
                   out->devices);
            }
        }
    }
...
static void audiodebug_process(struct tiny_audio_device *adev,struct str_parms *parms)
{
    ...
    if (ret >= 0) {
        ...
        if(((val & (AUDIO_DEVICE_OUT_WIRED_HEADSET | AUDIO_DEVICE_OUT_WIRED_HEADPHONE | AUDIO_DEVICE_OUT_SPEAKER | AUDIO_DEVICE_OUT_EARPIECE | AUDIO_DEVICE_OUT_BLUETOOTH_A2DP |
            ...
            ALOGI("adev_set_parameters test_stream_route want to set devices:0x%x mode:%d call_start:%d ",adev->out_devices,adev->mode,adev->call_start);
            select_devices_signal(adev);
            pthread_mutex_unlock(&adev->lock);
        } else {
            pthread_mutex_unlock(&adev->lock);
...
#ifdef AUDIOHAL_V4
            hal_audio_ctrl = fopen("/data/vendor/local/media/mmi.audio.ctrl","wb");
#else
...
static void adev_set_out_devices(struct audio_hw_device *dev, int device)
{
    struct tiny_audio_device *adev = (struct tiny_audio_device *)dev;
    int ret=-1;


    ALOGI("adev_set_out_devices device is %x,%x",adev->out_devices,device);
    pthread_mutex_lock(&adev->lock);
#ifdef AUDIOHAL_V4
    if (device & AUDIO_DEVICE_OUT_ALL)
        adev_check_out_devices(dev,device);
#endif

    if(adev->boot_completed==false){
        if(((AUDIO_DEVICE_OUT_WIRED_HEADPHONE|AUDIO_DEVICE_OUT_WIRED_HEADSET)&device)
            ||(AUDIO_DEVICE_OUT_ALL_SCO&device)){
            ALOGI("adev_set_out_devices connect devices, boot completed");
            adev->boot_completed=true;
        }else{
            check_boot_status(adev);
            if(adev->boot_completed==false){
...
#ifdef NXP_SMART_PA_HIDL
        FMSpkCheckAndClose(adev,device);
#endif
        if (device & AUDIO_DEVICE_OUT_ALL) {
            adev->out_devices &= ~AUDIO_DEVICE_OUT_ALL;
            adev->out_devices |= device;
            ...
            select_devices_signal(adev);
            if (AUDIO_MODE_IN_CALL == adev->mode || adev->voip_start || adev->call_start == 1)
                ret = at_cmd_route(adev);  //send at command to cp
            ALOGD("adev_set_out_devices want to set devices:0x%x mode:%d call_start:%d ",adev->out_devices,adev->mode,adev->call_start);
        } else {
            ALOGD("adev_set_out_devices invalid device\n");
        }
    }
    pthread_mutex_unlock(&adev->lock);
}
...
#ifdef AUDIOHAL_V4
static void adev_set_in_pop_mute(struct tiny_audio_device *adev)
{
    struct tiny_stream_in *in = NULL;
    struct listnode *item;
    struct listnode *item2;
...
static void adev_set_in_devices(struct audio_hw_device *dev, int device)
{
    struct tiny_audio_device *adev = (struct tiny_audio_device *)dev;
    int ret=-1;
    ALOGI("adev_set_in_devices device is %x,%x",adev->in_devices,device);
    pthread_mutex_lock(&adev->lock);
    adev_check_input_devices(dev,device);
...
static int adev_set_parameters(struct audio_hw_device *dev, const char *kvpairs)
{
    ...
    ALOGI("adev_set_parameters  kvpairs : %s", kvpairs);
    ...
    if(ret >= 0){
        ...
        if(val){
            ...
            if(val&(AUDIO_DEVICE_OUT_WIRED_HEADSET|AUDIO_DEVICE_OUT_WIRED_HEADPHONE)){
                adev->headset_connected=true;
            }
            pthread_mutex_unlock(&adev->lock);
         }
    }
    ...
    if(ret >= 0){
        ...
        if(!(val & AUDIO_DEVICE_BIT_IN) &&
            val &(AUDIO_DEVICE_OUT_WIRED_HEADSET|AUDIO_DEVICE_OUT_WIRED_HEADPHONE)){
            pthread_mutex_lock(&adev->lock);
            adev->headset_connected=false;
            pthread_mutex_unlock(&adev->lock);
        }
...
#endif
        LOG_I("fm_set_prestop:%d",val);
        pthread_mutex_lock(&adev->lock);
        adev->fm_pre_stop = val;
        pthread_mutex_unlock(&adev->lock);
    }
    ...
    if (ret >= 0) {
        ...
        if (adev->fm_type != AUD_FM_TYPE_LINEIN_3GAINS) {
            if(val) {
                set_fm_mute(adev, true);
            }
            else if(adev->fm_mute_apk == false &&
                adev->fm_volume != 0) {
                set_fm_mute(adev, false);
            }
            else {
                LOG_I("policy_force_fm_mute:force mute:%d, adev->fm_volume:%d,so can't unmute",adev->force_fm_mute,adev->fm_volume);
            }
        }
        pthread_mutex_unlock(&adev->lock);
        pthread_mutex_unlock(&adev->device_lock);
    ...
    ret = str_parms_get_str(parms, "set_fm_speaker", value, sizeof(value));
    if (ret >= 0) {
        val = atoi(value);
        ALOGI("set_fm_speaker:%d",val);
        pthread_mutex_lock(&adev->lock);
        adev->set_fm_speaker = val;
        pthread_mutex_unlock(&adev->lock);
    }
    ...
    if (ret >= 0) {
        ...
        ALOGI("media.audiofw.devices.dump:%s",value);
#ifdef AUDIOHAL_V4
        if(property_set("persist.vendor.media.audiofw.devices.dump",value) < 0) {
            ALOGE("property_set error:persist.vendor.media.audiofw.devices.dump");
        }
#else
        if(property_set("media.audiofw.devices.dump",value) < 0) {
            ALOGE("property_set error:media.audiofw.devices.dump");
        }
#endif
    }
...
#ifdef AUDIOHAL_V4
        ext_property_set("persist.vendor.media.dump.switch", value);
#else
    ...
    ret = str_parms_get_str(parms, "audio_nr_enable", value, sizeof(value));
    if (ret >= 0) {
        pthread_mutex_lock(&adev->lock);
        val = atoi(value);
        if(val){
            adev->forceuse_nr = true;
         }else{
            adev->forceuse_nr = false;
         }
         pthread_mutex_unlock(&adev->lock);
    }
    ...
    ret = str_parms_get_str(parms, "sprd_aec_on", value, sizeof(value));
    if (ret >= 0) {
        pthread_mutex_lock(&adev->lock);
        val = atoi(value);
        if(val == 1){
         ...
         pthread_mutex_unlock(&adev->lock);
         ALOGI("adev->sprd_aec_on is %d", adev->sprd_aec_on);
    }
    ret = str_parms_get_str(parms, "sprd_aec_effect_valid", value, sizeof(value));
    if (ret >= 0) {
        pthread_mutex_lock(&adev->lock);
        val = atoi(value);
        if (val == 1) {
         ...
         pthread_mutex_unlock(&adev->lock);
         ALOGI("adev->sprd_aec_effect_valid is %d", adev->sprd_aec_effect_valid);
    }
 #if VOIP_DSP_PROCESS
...
#ifdef SPRD_AUDIO_HIDL_CLIENT
    ret = str_parms_get_str(parms, "AudioNpiInit", value, sizeof(value));
    if (ret >= 0) {
        pthread_mutex_lock(&adev->lock);
        adev->boot_completed=true;
#ifdef DYNAMIC_AUDIO_ENCRYPT
...
#ifdef DYNAMIC_AUDIO_ENCRYPT
    ret = str_parms_get_str(parms, "AudioTesterInit", value, sizeof(value));
    if (ret >= 0) {
        pthread_mutex_lock(&adev->lock);
        check_vbc_eq(adev);
        audiopara_tuning_manager_create(adev);
        pthread_mutex_unlock(&adev->lock);
        ALOGI("AudioTesterInit");
    }
#endif
#endif

#ifdef AUDIOHAL_V4
    ret = str_parms_get_str(parms, "boot_complete", value, sizeof(value));
    if (ret >= 0) {
        if(strcmp(value, "true") == 0) {
            pthread_mutex_lock(&adev->lock);
            if(adev->boot_completed==false){
    ...
    if (ret >= 0) {
        ...
        if(checkvalid && val != adev->fm_volume)
        {
            ...
            if (adev->fm_type != AUD_FM_TYPE_LINEIN_3GAINS) {
                ...
                ALOGI("adev_set_parameters fm_open: %d, fm_volume_tbl[%d] %x", adev->fm_open, val, gain);
                if((false == adev->force_fm_mute) && (false == adev->fm_mute_apk)){
                    usleep(10000);
                    if(val !=0) {
                        set_fm_mute_noramp(dev, false);
                    } else {
                        set_fm_mute_noramp(dev, true);
                    }
                }
            }
        pthread_mutex_unlock(&adev->lock);
   ...
   if(ret >= 0){
    ...
    if( ret >= 0 ){
        ...
        if (adev->fm_type != AUD_FM_TYPE_LINEIN_3GAINS) {
            ...
            pthread_mutex_lock(&adev->lock);
            if( val == 1){
                adev->fm_mute_apk = true;
                ret = set_fm_mute(adev, 1);
            } else {
                adev->fm_mute_apk = false;
                if (false == adev->force_fm_mute &&
                    adev->fm_volume != 0) {
                    ret = set_fm_mute(adev, 0);
                } else {
                    LOG_I("FM state is force mute:%d, adev->fm_volume:%d,so can't unmute",adev->force_fm_mute,adev->fm_volume);
                }
            }
            pthread_mutex_unlock(&adev->device_lock);
            pthread_mutex_unlock(&adev->lock);
...
#ifdef HAC_TTY_SUPPORT
    ret = str_parms_get_str(parms, "HACSetting", value, sizeof(value));//add for hac dvice
    if( ret >= 0 ) {
        if (strncmp(value, "ON",2) == 0) {
            adev->hac_dev = HAC_DEV;
            pthread_mutex_lock(&adev->lock);
            at_cmd_route(adev);
            pthread_mutex_unlock(&adev->lock);
        }
        if (strncmp(value, "OFF",3) == 0) {
            adev->hac_dev = HAC_TTY_NULL;
            pthread_mutex_lock(&adev->lock);
            at_cmd_route(adev);
            pthread_mutex_unlock(&adev->lock);
        }
    ...
    ret = str_parms_get_str(parms, "tty_mode", value, sizeof(value));//add for tty device
    if( ret >= 0 ) {
        if(strncmp(value, "tty_full", 8) == 0) {
            adev->tty_dev = TTY_FULL_DEV;
            pthread_mutex_lock(&adev->lock);
            at_cmd_route(adev);
            pthread_mutex_unlock(&adev->lock);
        }
        if(strncmp(value, "tty_vco", 7) == 0) {
            adev->tty_dev = TTY_VCO_DEV;
            pthread_mutex_lock(&adev->lock);
            at_cmd_route(adev);
            pthread_mutex_unlock(&adev->lock);
        }
        if(strncmp(value, "tty_hco", 7) == 0) {
            adev->tty_dev = TTY_HCO_DEV;
            pthread_mutex_lock(&adev->lock);
            at_cmd_route(adev);
            pthread_mutex_unlock(&adev->lock);
        }
        if (strncmp(value, "tty_off", 7) == 0) {
            adev->tty_dev = HAC_TTY_NULL;
            pthread_mutex_lock(&adev->lock);
            at_cmd_route(adev);
            pthread_mutex_unlock(&adev->lock);
        }
    ...
    if (ret > 0) {
        val = atoi(value);
        if(val == 1){
            ALOGI("%s, record tone by output, value:%d, call_connected:%d", __FUNCTION__,val, adev->call_connected);
            ret = adev_record_tone_start(adev, NULL);
        } else if (val == 0) {
            ALOGI("%s, record tone by output, value:%s", __FUNCTION__,value);
    ...
    if (ret > 0) {
        ...
        if(val != 0){
            ALOGI("%s, record tone by output, value:%d, call_connected:%d, adev->record_tone_info.fd:%d",
    ...
    if (ret > 0) {
        ...
        val = atoi(value);
        ALOGI("%s, mixer_route, val:%d", __FUNCTION__, val);
        ...
        const char *keys)
{
    ...
    if (strcmp(keys, "point_info") == 0) {
        ALOGI("keys: %s",keys);
        return get_pointinfo_hal(adev->cp_nbio_pipe);
        ...
        if (pFileVbc==NULL) {
           ALOGW(" %s open vbc failed ",__func__);
           return NULL;
        }
        pFileCodec = fopen ("/proc/asound/sprdphone/sprd-codec", "rb" );
        if (pFileCodec==NULL) {
           ALOGW(" %s open codec failed ",__func__);
           if(pFileVbc) {
                fclose(pFileVbc);
           }
           return NULL;
        ...
        //read vbc reg
        reads = fread(dumpbuffer, max_size,1,pFileVbc);
        if (reads) {
            ALOGI("pFileVbc readsize is %d", reads);
        }
        ...
        reads = fread(tmpbuffer, max_size -read_size ,1,pFileCodec);
        ALOGD("%s, reads:%d", tmpbuffer, reads);
    ...
    } else if (strcmp(keys, "isFMPrestop") == 0) {
        char deltaStr[32] = {0};
        char *str = NULL;
        snprintf(deltaStr, sizeof(deltaStr), "isFMPrestop=%d", adev->fm_pre_stop);
        str = strdup(deltaStr);
        return str;
    }

    if (strcmp(keys, "headphonestate") == 0) {
        char deltaStr[200] = {0};
        char *str = NULL;
        uint32_t device = 0;
        int usb_cardnum = -1;
...
{
    struct tiny_audio_device *adev = (struct tiny_audio_device *)dev;
    ALOGI("adev_set_master_volume in...devices:0x%x ,volume:%f ",adev->out_devices,volume);
...
static int do_speaker_mute(struct tiny_audio_device *adev , bool mute)
{
    ...
    ALOGI("%s: %s speaker ",__func__,mute?"mute":"unmute");
        ...
        audio_source_t source)
{
    ...
    ALOGI("adev_open_input_stream,devices=0x%x,sample_rate=%d, channel_count=%d source:%d",
            devices, config->sample_rate, channel_count,source);
    ...
    if(AUDIO_SOURCE_VOICE_RECOGNITION==source){
    ...
    in->source = source;
    in->source_to_set = source;

#ifdef NXP_SMART_PA_HIDL
    if(AUDIO_SOURCE_FM_TUNER == in->source) {
        in->is_fm_ul = true;
        if (1 == in->requested_channels) {
            int dwSize = get_input_buffer_size(in->requested_rate, AUDIO_FORMAT_PCM_16_BIT,
            in->requested_channels);
...
#endif
    *stream_in = &in->stream;
    ALOGI("Successfully, adev_open_input_stream.");
    return 0;
        ...
        struct audio_stream_in *stream)
{
    ...
    if (in->fmUlDlHandle) {
        ALOGI("fm_pcm_open pcm_in driver ");
        fm_pcm_close(in->fmUlDlHandle);
        in->fmUlDlHandle = NULL;
    }
...
static int adev_get_microphones(const struct audio_hw_device *dev,
                                ...
                                size_t *mic_count) {
    struct tiny_audio_device *adev = (struct tiny_audio_device *)dev;
    ALOGI("adev_get_microphones enter:%d",*mic_count);

    pthread_mutex_lock(&adev->lock);
    int ret = platform_get_microphones(adev->audio_data, mic_array, mic_count);
    pthread_mutex_unlock(&adev->lock);
    ALOGI("adev_get_microphones exit:%d",*mic_count);
    return ret;
}
#endif
...
static int adev_dump(const audio_hw_device_t *device, int fd)
{
    ...
    snprintf(buffer,(DUMP_BUFFER_MAX_SIZE-1),
        "other status:dump_flag:0x%x nr enable flag:0x%x,0x%x log_level:%d\n",
        adev->dump_flag,
        (unsigned int)adev->nr_mask,
        (unsigned int)adev->enable_other_rate_nr,
        log_level);
    AUDIO_DUMP_WRITE_STR(fd,buffer);
    memset(buffer,0,sizeof(buffer));
...
/* dump input */
{
    ...
    snprintf(buffer,(DUMP_BUFFER_MAX_SIZE-1),"microphones_count:%d \nadev_dump <===\n",
        adev->audio_data->microphones_count);
#else
    snprintf(buffer,(DUMP_BUFFER_MAX_SIZE-1),"\nadev_dump <===\n");
#endif
    AUDIO_DUMP_WRITE_STR(fd,buffer);
...
#ifdef AUDIOHAL_V4
static void audio_port_config_log(struct audio_port_config *config, const char *tag_str){
    int numValues=0;
    int i=0;

    switch(config->type){
        case AUDIO_PORT_TYPE_DEVICE:
            ALOGI("AUDIO_PORT_TYPE_DEVICE %s device type:0x%x hw_module:%d\n",tag_str
                ,config->ext.device.type,config->ext.device.hw_module);
            break;

        case AUDIO_PORT_TYPE_MIX:
            ALOGI("AUDIO_PORT_TYPE_MIX %s handle:%d hw_module:%d stream:%d  source:%d\n",tag_str
        ...
        case AUDIO_PORT_TYPE_SESSION:
            ALOGI("AUDIO_PORT_TYPE_SESSION %s session:%d\n",tag_str,
                config->ext.session.session);
            break;

        case AUDIO_PORT_TYPE_NONE:
            ALOGI("AUDIO_PORT_TYPE_NONE %s session:%d\n",tag_str,
                config->ext.session.session);
            break;

        default:
            ALOGI("AUDIO_PORT_TYPE_INVALID %s\n",tag_str);
            break;
    }

    ALOGD("%s audio_port_config id:%d role:%d type:%d config_mask:%d sample_rate:%d channel_mask id:%d format:%d\n",
        tag_str,
        config->id, config->role,config->type, config->config_mask,
        config->sample_rate,config->channel_mask,  config->format);

    ALOGD("%s gain index:%d mode:0x%x channel_mask:0x%x ramp_duration_ms:%d\n",tag_str,
...
static int adev_create_audio_patch(struct audio_hw_device *dev,
    ...
    for (int i = 0; i < num_sinks; i++) {
        LOG_I("%s: sink[%d] type=%d address=%s", __func__, i, sinks[i].type,
                ...
                : "N/A");
        LOG_I(":device:%x",sinks_device_type |=sinks[i].ext.device.type);
    ...
    if ((num_sinks == 1 && sinks[0].type == AUDIO_PORT_TYPE_MIX
        ...
        && sources_device_type > 0)) {
        adev_set_in_devices(dev,sources_device_type);
        adev_set_in_pop_mute(adev);
    }

    if ((num_sources == 1 && sources[0].type == AUDIO_PORT_TYPE_MIX
        &&sinks_device_type>0)){
        adev_set_out_devices(dev,sinks_device_type);
    }
    ...
    if (num_sources == 1 && num_sinks == 1 &&
        ...
        *handle = 42;
        ALOGD("%s: handle: %d loop", __func__, *handle);
    }else{
...
static void adev_free_modem_mem(struct tiny_audio_device *adev)
{
    ...
    if (NULL != adev->cp->i2s_fm_bt_loop) {
        if (NULL != adev->cp->i2s_fm_bt_loop->ctl_name) {
            free(adev->cp->i2s_fm_bt_loop->ctl_name);
            adev->cp->i2s_fm_bt_loop->ctl_name = NULL;
        }
        if (NULL != adev->cp->i2s_fm_bt_loop->strval) {
            free(adev->cp->i2s_fm_bt_loop->strval);
            adev->cp->i2s_fm_bt_loop->strval = NULL;
        }
        free(adev->cp->i2s_fm_bt_loop);
        adev->cp->i2s_fm_bt_loop = NULL;
    }
}
...
#ifdef AUDIOHAL_V4
    if(NULL!=adev->audio_data){
        free(adev->audio_data);
        adev->audio_data=NULL;
    }
#endif
        ...
        const XML_Char **attr)
{
    ...
    if (!name) {
        ALOGW("unnamed entry %s, %d", elem, i);
        return;
    }
...
int load_fm_volume(struct tiny_audio_device *adev)
{
    ...
    for (i = 1; i < FM_VOLUME_MAX; i++) {
        //16 is used for fm record dg gain
        if(i < 16) {
            ...
            fm_headset_volume_tbl[i] = aud_params_ptr->audio_nv_arm_mode_info.tAudioNvArmModeStruct.app_config_info_set.app_config_info[15].arm_volume[i+1];
        }
    ...
    for (i = 1; i < FM_VOLUME_MAX; i++) {
        ...
        if(i < 16) {
            ...
            fm_speaker_volume_tbl[i] = aud_params_ptr->audio_nv_arm_mode_info.tAudioNvArmModeStruct.app_config_info_set.app_config_info[15].arm_volume[i+1];
        }
...
static int init_rec_process(int rec_mode, int sample_rate)
{
    ...
    ALOGI("rec_mode(%d), sample_rate(%d)", rec_mode, sample_rate);
...
static int aud_rec_do_process(void * buffer,size_t bytes,int channels,void * tmp_buffer,size_t tmp_buffer_bytes)
{
    ...
    if (temp_buf && (tmp_buffer_bytes >= 2)) {
        ...
        if(1==channels){
            ...
            ALOGI("aud_rec_do_process do not supprot channels=%d",channels);
         }
...
static void *stream_routing_thread_entry(void * param)
{
    ...
    setpriority(PRIO_PROCESS, 0, ANDROID_PRIORITY_AUDIO);
    set_sched_policy(0, SP_FOREGROUND);
    prctl(PR_SET_NAME, (unsigned long)"Audio Stream Routing Thread", 0, 0, 0);
    ...
    ALOGI("stream_routing_thread_entry exit!!!");
    return 0;
}
...
static void *voice_command_thread_entry(void * param)
{
    ...
    setpriority(PRIO_PROCESS, 0, ANDROID_PRIORITY_AUDIO);
    set_sched_policy(0, SP_FOREGROUND);
    prctl(PR_SET_NAME, (unsigned long)"Audio Voice Command Thread", 0, 0, 0);
        ...
        const XML_Char **attr)
{
            ...
            if (strstr(attr[i], "ctl_file") > 0) {
                if((strlen(attr[i+1]) +1) <= I2S_CTL_PATH_MAX) {
                 if( state->i2s_ctl_info->p_ctlr_node_head== NULL ) {
                        ...
                        if(NULL==p_ctlr_node){
                            ALOGE("%s line:%d malloc:%d failed",__func__,__LINE__,sizeof(ctrl_node));
                            continue;
                        }
                        state->i2s_ctl_info->p_ctlr_node_head = p_ctlr_node;
                    } else {
                        if (p_ctlr_node == NULL) {
                            ALOGE("%s line:%d failed and return",__func__,__LINE__);
                            break;
                        }
                        p_ctlr_node->next = malloc(sizeof(ctrl_node));
                        if(NULL==p_ctlr_node->next){
                            ALOGE("%s line:%d malloc:%d failed",__func__,__LINE__,sizeof(ctrl_node));
                            continue;
                        }
                        p_ctlr_node = p_ctlr_node->next;
                   ...
                   if(!p_ctlr_node) {
                        ALOGE("%s line:%d failed and return:p_ctlr_node is null",__func__,__LINE__);
                        break;
                   }
    ...
    state->i2s_ctl_info++;
        ...
        if (strcmp(attr[0], "value") == 0) {
            ...
            ALOGI("mic_default_channel is '%d", modem->mic_default_channel);
        } else {
            ALOGI("no mic_default_channel!");
    ...
    } else if (strcmp(tag_name, "i2s_fm_bt_loop") == 0) {
        modem->i2s_fm_bt_loop = malloc(sizeof(struct route_setting));
        if(modem->i2s_fm_bt_loop == NULL) {
            ALOGE("malloc i2s_fm_bt_loop err ");
        } else {
            modem->i2s_fm_bt_loop->ctl_name = NULL;
            modem->i2s_fm_bt_loop->strval = NULL;
            LOG_I("i2s_switch_fm :%s-->%s",attr[1],attr[3]);
            if (strcmp(attr[0], "ctl") == 0) {
                modem->i2s_fm_bt_loop->ctl_name = strdup(attr[1]);
            }
            if (strcmp(attr[2], "value") == 0) {
                modem->i2s_fm_bt_loop->intval = atoi((char *)attr[3]);
            }
        }
   ...
   else if (strcmp(tag_name, "debug") == 0) //parse debug info
   {
    ...
    else if (strcmp(tag_name, "debuginfo") == 0) //parse debug info
    {
    ...
    }else if (strcmp(tag_name, "nr_support") == 0) //parse nr support info
       {
        ...
        parse_microphone_characteristic(attr,state->adev->audio_data);
    }
#endif
    else{
...
static void *audiopara_tuning_thread_entry(void * param)
{
    ...
    setpriority(PRIO_PROCESS, 0, ANDROID_PRIORITY_AUDIO);
    set_sched_policy(0, SP_FOREGROUND);
    prctl(PR_SET_NAME, (unsigned long)"AudioPara Tuning Thread", 0, 0, 0);
            ...
            if(ENG_PHONEINFO_OPS & ops_bit) {
                ...
                if(pmem!=NULL){
                    ...
                    if (send_fd_aud == -1) {
                        ...
                        if(result){
                            ALOGE("write %d bytes error:(%s)",sizeof(int),strerror(errno));
                        }
                        result = write(send_fd_aud ,pmem,length);
                        if(result){
                            ALOGE("write %d bytes error:(%s)",length,strerror(errno));
                        }
                        close(send_fd_aud );
...
#ifdef DYNAMIC_AUDIO_ENCRYPT
    if(false==pMgr->is_created){
        ALOGW("audiopara_tuning_manager_close tunning thread is not running");
        return 0;
    }
#endif
...
static int audiopara_tuning_manager_create(struct tiny_audio_device *adev)
{
    int ret=0;
#ifdef DYNAMIC_AUDIO_ENCRYPT
    if(false==adev->tunningMgr.is_created)
#endif
    {
        pthread_attr_t attr;
        pthread_attr_init(&attr);
        ...
        ALOGI("audiopara_tuning_manager_create Enter");
        ret = pthread_create(&adev->tunningMgr.thread_id, &attr,
                audiopara_tuning_thread_entry, (void *)adev);
        if (ret) {
            ALOGE("audiopara_tuning_manager_create falied, ret is %d", ret);
            return ret;
        }
#ifdef DYNAMIC_AUDIO_ENCRYPT
        adev->tunningMgr.is_created=true;
#endif
    }
    return ret;
}
...
#ifdef AUDIOHAL_V4
    if (!property_get("ro.vendor.audio.product.hardware", value, "0"))
#else
...
int adev_record_tone_start(struct tiny_audio_device *adev, char* file_name)
{
    ...
    if(!adev->call_connected){
        ALOGI("yaye %s, fail! call not connected!", __FUNCTION__);
        pthread_mutex_unlock(&adev->lock);
        return ret;
    }
...
#ifdef DYNAMIC_AUDIO_ENCRYPT
#define EQ_LOAD_CNT_MAX 50
#else
#define EQ_LOAD_CNT_MAX 4
#endif
    ...
    if (ret != 0) {
        ...
        do {
              ...
              if (ret == 0)
                    break;
              usleep(200000);
        } while (eq_load_cnt < EQ_LOAD_CNT_MAX);
        ALOGI("eq_load_cnt = %d", eq_load_cnt);
        eq_load_cnt = 0;
        release_wakeLock();
...
static bool check_data_ready(void *dev){
    ...
    if(false==adev->data_ready){
        if((property_get("ro.crypto.state", val, NULL))>=0){
            if(0 == strncmp(val,"encrypted",strlen("encrypted"))){
                memset(val,0,sizeof(val));
                if((property_get("ro.crypto.type", val, NULL))>=0){
                    if(0 == strncmp(val,"block",strlen("block"))){
                        memset(val,0,sizeof(val));
                        if((property_get("vold.decrypt", val, NULL))>=0){
                            if(0 == strncmp(val,"trigger_restart_framework",strlen("trigger_restart_framework"))){
                                // data is ready now
                                adev->data_ready=true;
                                ALOGI("check_data_ready Success");
                                if(false==check_vbc_eq(adev)){
                                    ALOGI("check_data_ready check_vbc_eq failed");
                                    adev->data_ready=false;
                                }
                                audiopara_tuning_manager_create(adev);
#ifdef AUDIO_DEBUG
                                check_ext_control(adev);
#endif
                                return true;
                            }
                        }
                    ...
                    if(0 == strncmp(val,"file",strlen("file"))){
                        // data is ready now
                        adev->data_ready=true;
                        ALOGI("check_data_ready encrypted file Success");
                        if(false==check_vbc_eq(adev)){
                            ALOGI("check_data_ready file check_vbc_eq failed");
                            adev->data_ready=false;
                        }
                        audiopara_tuning_manager_create(adev);
#ifdef AUDIO_DEBUG
            ...
            else if(0 == strncmp(val,"unencrypted",strlen("unencrypted"))){
                ALOGI("check_data_ready unencrypted");
                adev->data_ready=true;
                if(false==check_vbc_eq(adev)){
                    ALOGI("check_data_ready check_vbc_eq failed");
                    adev->data_ready=false;
            ...
            ALOGI("check_data_ready get ro.crypto.state failed");
            adev->data_ready=true;
            if(false==check_vbc_eq(adev)){
                ALOGI("check_data_ready check_vbc_eq failed");
                adev->data_ready=false;
...
#endif
bool check_vbc_eq(void *dev){
    struct tiny_audio_device *adev=(struct tiny_audio_device *)dev;
    if((access("/data/vendor/local/media/vbc_eq", R_OK) != 0)
    ||(adev->eq_available==false)){
        ALOGI("check_vbc_eq init_vbc_eq again :%d",adev->eq_available);
        init_vbc_eq(adev);
    }
    return adev->eq_available;
}
...
#ifdef AUDIOHAL_V4
    adev->audio_data = calloc(1, sizeof(struct audio_platform_data));
    if (!adev)
    {
        ALOGE("malloc audio_platform_data fail, size:%d", sizeof(struct audio_platform_data));
        pthread_mutex_unlock(&adev_init_lock);
        return -ENOMEM;
    }
    memset(adev->audio_data, 0, sizeof(struct audio_platform_data));
#endif
    s_adev = adev;
...
vb_ctl_modem_monitor_open (adev);
    ...
    adev->ext_contrl->entryMgr.isExit = true;
    adev->tunningMgr.fd=-1;
#ifdef DYNAMIC_AUDIO_ENCRYPT
    adev->tunningMgr.is_created=false;

    check_data_ready(adev);
    if(false==adev->data_ready){
        ALOGI("adev_open:data is not ready");
#ifdef AUDIO_DEBUG
        create_ext_pipe();
#endif
    }
...
LOG_I("adev_open:cp_mixer TG:%p W:%p CSFB:%p",adev->cp_mixer[CP_TG],adev->cp_mixer[CP_W],adev->cp_mixer[CP_CSFB]);
    ...
    if (sysinfo(&info)) {
        ...
        ALOGI( "adev_open get sysinfo, uptime:%ld\n", info.uptime);
        if(info.uptime>60){
            ALOGI("adev_open set boot completed");
            adev->boot_completed=true;
        }
    }
