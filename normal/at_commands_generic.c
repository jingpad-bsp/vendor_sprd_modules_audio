...
static int at_cmd_route(struct tiny_audio_device *adev)
{
    ...
    if ((adev->mode != AUDIO_MODE_IN_CALL) && (!adev->voip_start) && (!adev->call_start)) {
    ...
    if (adev->out_devices & (AUDIO_DEVICE_OUT_WIRED_HEADSET | AUDIO_DEVICE_OUT_WIRED_HEADPHONE)) {
        if (adev->tty_dev == HAC_TTY_NULL) {
            if(!headset_no_mic())
                at_cmd = "AT+SSAM=2";
            else
                at_cmd = "AT+SSAM=4";
        } else {
            if (adev->tty_dev == TTY_FULL_DEV)
                at_cmd = "AT+SSAM=9";
            if (adev->tty_dev == TTY_VCO_DEV)
                at_cmd = "AT+SSAM=10";
            if (adev->tty_dev == TTY_HCO_DEV)
                at_cmd = "AT+SSAM=11";
        }
