...
static int do_parse(AUDIO_TOTAL_T *audio_params_ptr, unsigned int params_size)
{
    ...
    fd_dest_paras = open(STORED_VBC_EFFECT_PARAS_PATH, O_WRONLY | O_CREAT | O_TRUNC, 0600);
    if (0 > fd_dest_paras) {
        free(fw_header);
        free(effect_profile);
    ...
    ALOGI("fd_dest_paras(%d), header_len(%d), profile_len(%d)", fd_dest_paras,
    ...
    for (i=0; i<fw_header->num_profile; i++) {
        ...
        write(fd_dest_paras, effect_profile, sizeof(struct vbc_eq_profile));
    }
    fsync(fd_dest_paras);
    close(fd_dest_paras);
    free(fw_header);
