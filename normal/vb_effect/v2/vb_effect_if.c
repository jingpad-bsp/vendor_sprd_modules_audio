...
#ifdef AUDIOHAL_V4
#define STORED_VBC_EFFECT_PARAS_PATH    "/data/vendor/local/media/vbc_eq"
#else
...
static int do_parse(AUDIO_TOTAL_T *audio_params_ptr, unsigned int params_size)
{
    ...
    fd_dest_paras = open(STORED_VBC_EFFECT_PARAS_PATH, O_WRONLY | O_CREAT | O_TRUNC, 0660);
    if (0 > fd_dest_paras) {
        free(fw_header);
        free(effect_profile);
        free(vbc_da_effect_profile);
    ...
    ALOGI("fd_dest_paras(%d), header_len(%d), da_profile_len(%d),ad_profile_len(%d)", fd_dest_paras,
            sizeof(struct vbc_fw_header), sizeof(struct vbc_da_eq_profile),sizeof(struct vbc_ad_eq_profile));
    //write dest file header
    write(fd_dest_paras, fw_header, sizeof(struct vbc_fw_header));
    ...
    for (i=0; i<s_audio_mode_num; i++) {
        ...
        lseek(fd_dest_paras,i*sizeof(struct vbc_da_eq_profile)+sizeof(struct vbc_fw_header), SEEK_SET);
        write(fd_dest_paras, vbc_da_effect_profile, sizeof(struct vbc_da_eq_profile));
        ...
        lseek(fd_dest_paras,sizeof(struct vbc_fw_header)+i*sizeof(struct vbc_ad_eq_profile)+s_audio_mode_num*sizeof(struct vbc_da_eq_profile), SEEK_SET);
        write(fd_dest_paras, vbc_ad01_effect_profile, sizeof(struct vbc_ad_eq_profile));
        ...
        lseek(fd_dest_paras,sizeof(struct vbc_fw_header)+i*sizeof(struct vbc_ad_eq_profile)+s_audio_mode_num*sizeof(struct vbc_da_eq_profile)+s_audio_mode_num*sizeof(struct vbc_ad_eq_profile), SEEK_SET);
        write(fd_dest_paras, vbc_ad23_effect_profile, sizeof(struct vbc_ad_eq_profile));
    }
    fsync(fd_dest_paras);
    close(fd_dest_paras);
    free(fw_header);
...
int create_vb_effect_params(void)
{
    ...
    if(chmod(STORED_VBC_EFFECT_PARAS_PATH, S_IRUSR|S_IWUSR|S_IRGRP|S_IWGRP) != 0) {
        ALOGE("%s Cannot set RW to \"%s\": %s", __FUNCTION__,STORED_VBC_EFFECT_PARAS_PATH, strerror(errno));
    }
