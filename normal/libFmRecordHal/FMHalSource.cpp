 ...
 bool        FMHalThread:: threadLoop()
{
    ...
    while (!exitPending()) {
		...
		if(exit_var) {
            ...
            if(pcmHandle == NULL) {
                ...
                if(card < 0){
                    ...
                    pcmHandle = pcm_open(card, 0, PCM_IN|PCM_MMAP|PCM_NOIRQ|PCM_MONOTONIC , &pcm_cfg);
#else
        ...
        //ALOGD("pcm_read threadloop in");
            if(pcmHandle) {
#ifdef RECORD_USE_MMAP
                reval=pcm_mmap_read(pcmHandle,readBuf,readSize);
#else
...
#ifdef RECORD_USE_MMAP
             pcmHandle = pcm_open(card, 0, PCM_IN|PCM_MMAP|PCM_NOIRQ|PCM_MONOTONIC, &cfg);
#else
