...
#define AGDSP_CPROC_ROOT        "/sys/devices/platform/audiocp_boot/"
#define AGDSP_CPROC_START        "/sys/devices/platform/audiocp_boot/start"
#define AGDSP_CPROC_STOP        "/sys/devices/platform/audiocp_boot/stop"
#ifdef BM_DEV_ENABLE
#define BM_DEV                     "/dev/sprd_bm"
#endif /* BM_DEV_ENABLE */
#define PERSIS_IMAGE_ROOT_PATH    "ro.vendor.product.partitionpath"
...
int write_proc_file(char *file, UNUSED_ATTR int offset, char *string)
{
    ...
    do {
        ...
        if (fd < 0) {
            AGDSP_LOGE("write_proc_file open file %s, error: %s", file, strerror(errno));
            usleep(20000);
            retry++;
        }
    } while(fd < 0 && retry < 6);
...
int load_sipc_image(char *fin, int offsetin, char *fout, int offsetout, int size)
{
    ...
    char buf[AUDCP_BOOT_EACH_COPY_SIZE];
...
static int get_modem_load_info(char *file, int num, void *info)
{
    int fd, ret;
...
void agdsp_boot(void)
{
    ...
    /* get read path */
    property_get(PERSIS_IMAGE_ROOT_PATH, path_read, "not_find");
    ...
    strcpy(ldinfo.path_r, path_read);
