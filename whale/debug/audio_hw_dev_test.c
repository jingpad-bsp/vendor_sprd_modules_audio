...
static void *in_test_thread(void *args){
    ...
    struct tiny_stream_in * in_stream=(struct tiny_stream_in *)in_test->stream;
    ...
    while((!in_test->is_exit) && is_usecase(adev->dev_ctl, UC_AUDIO_TEST)){
        ret=in_stream->stream.read(&in_stream->stream, buffer, size);
        if (ret<0){
...
static void * open_in_devices_test(struct tiny_audio_device *adev,int devices,int sample_rate,int channel){
    ...
    struct audio_stream_in *in_stream_tmp=NULL;
    ...
    if(list_empty(&adev->active_input_list)){
        ...
        ret=adev_open_input_stream(&adev->hw_device,0,0,&config,&in_stream_tmp,AUDIO_INPUT_FLAG_NONE,NULL,AUDIO_SOURCE_MIC);
        if(0!=ret){
    ...
    in_stream=(struct tiny_stream_in *)in_stream_tmp;
...
static int audio_loop_devices_test_stop(struct tiny_audio_device *adev,UNUSED_ATTR struct str_parms *parms){
    struct dev_test_t *dev_test = (struct dev_test_t *) &adev->dev_test;
    struct stream_test_t *in_test = (struct stream_test_t *) &dev_test->in_test;
    struct stream_test_t *out_test = (struct stream_test_t *) &dev_test->out_test;
    int ret = -1;
    ...
    if(NULL!=in_test->stream){
        adev->hw_device.close_input_stream(&adev->hw_device,in_test->stream);
        in_test->stream=NULL;
    }
...
int audio_in_devices_test_stop(struct tiny_audio_device *adev,UNUSED_ATTR struct str_parms *parms){
    struct dev_test_t *dev_test = (struct dev_test_t *) &adev->dev_test;
    struct stream_test_t *in_test = (struct stream_test_t *) &dev_test->in_test;
    int ret = -1;
...
static int audio_out_devices_test_stop(struct tiny_audio_device *adev,UNUSED_ATTR struct str_parms *parms){
    struct dev_test_t *dev_test = (struct dev_test_t *) &adev->dev_test;
    struct stream_test_t *out_test = (struct stream_test_t *) &dev_test->out_test;
    int ret = -1;
...
int audio_out_devices_test(void * dev,struct str_parms *parms,int is_start,UNUSED_ATTR char * val){
    int ret=-1;
    ...
    if(is_start){
        ...
        parse_audio_config(&adev->dev_ctl->config);
    }
    return ret;
}

int audio_in_devices_test(void * dev,struct str_parms *parms,int is_start,UNUSED_ATTR char * val){
    int ret=-1;
    struct tiny_audio_device *adev=(struct tiny_audio_device *)dev;
    LOG_D("%s %d",__func__,__LINE__);
    if(is_start){
...
int sprd_audioloop_test(void * dev,struct str_parms *parms,int is_start,UNUSED_ATTR char * val){
    int ret=-1;
    struct tiny_audio_device *adev=(struct tiny_audio_device *)dev;
    bool support_24bits=adev->dev_ctl->config.support_24bits;
    LOG_D("%s %d",__func__,__LINE__);
