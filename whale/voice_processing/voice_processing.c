...
// libsprdvoiceprocessing.<product_name>.so if needed.
//------------------------------------------------------------------------------
...
static const effect_descriptor_t sprd_default_aec_descriptor = {
        { 0x7b491460, 0x8d4d, 0x11e0, 0xbd61, { 0x00, 0x02, 0xa5, 0xd5, 0xc5, 0x1b } }, // type
        { 0xa4a00091, 0x9087, 0x426b, 0x9d60, { 0xaf, 0x3a, 0x70, 0xf6, 0xa2, 0x0a } }, // uuid
...
static void session_set_enabled(struct session_s *session, uint32_t id, bool enabled);
...
static int effect_set_state(struct effect_s *effect, uint32_t state)
{
    ...
    case EFFECT_STATE_INIT:
        ...
        case EFFECT_STATE_ACTIVE:
            session_set_enabled(effect->session, effect->id, false);
    ...
    case EFFECT_STATE_ACTIVE:
        ...
        case EFFECT_STATE_ACTIVE:
            ...
            break;
        case EFFECT_STATE_CONFIG:
            session_set_enabled(effect->session, effect->id, true);
            break;
        default:
            BAD_STATE_ABORT(effect->state, state);
        }
                                 ...
                                 effect_handle_t  *interface)
{
    ...
    if (status < 0)
        goto error;
    audioclient_setParameters("sprd_aec_effect_valid=1");
                                  ...
                                  struct effect_s *fx)
{
    ...
    audioclient_setParameters("sprd_aec_effect_valid=0");
    return 0;
}
...
static void session_get_config(struct session_s *session, effect_config_t *config)
{
    ...
    config->inputCfg.mask = config->outputCfg.mask =
            (EFFECT_CONFIG_SMP_RATE | EFFECT_CONFIG_FORMAT);
}
...
static void session_set_enabled(struct session_s *session, uint32_t id, bool enabled)
{
    if (enabled) {
        if(session->enabled_msk == 0) {
            ...
            audioclient_setParameters("sprd_aec_on=1");
        }
        ...
        if(session->enabled_msk == 0) {
            /* do last enable here */
            audioclient_setParameters("sprd_aec_on=0");
        }
    }
    ALOGV("session_set_enabled() id %d, enabled %d enabled_msk %08x",
...
{
    ALOGI("aec:lib_create: uuid: %08x session %d IO: %d", uuid->timeLow, sessionId, ioId);
    ...
    if(status!=0){
        ALOGE("%s line:%d status:%d",__func__,__LINE__,status);
        if (pInterface != NULL){
          *pInterface = (effect_handle_t)NULL;
        }
    }
    return status;
}
...
audio_effect_library_t AUDIO_EFFECT_LIBRARY_INFO_SYM = {
    ...
    .name = "sprd Audio Preprocessing Library",
