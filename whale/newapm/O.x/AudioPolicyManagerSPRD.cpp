                                             ...
                                             audio_session_t session)
{
    ...
    if (outputDesc->mPolicyMix != NULL) {
     ...
     if((!isFmMute) && (stream == AUDIO_STREAM_DTMF || stream ==AUDIO_STREAM_ENFORCED_AUDIBLE || stream==AUDIO_STREAM_RING ||
            stream==AUDIO_STREAM_ALARM || stream == AUDIO_STREAM_NOTIFICATION || stream == AUDIO_STREAM_VOICE_CALL)){
        if((outputDesc->mRefCount[AUDIO_STREAM_DTMF] == 0) || (outputDesc->mRefCount[AUDIO_STREAM_ENFORCED_AUDIBLE]==0) || (outputDesc->mRefCount[AUDIO_STREAM_RING]==0) ||
