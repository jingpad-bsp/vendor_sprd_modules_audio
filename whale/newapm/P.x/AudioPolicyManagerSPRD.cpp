...
//      startReadingThread();
    ...
    if(hasPrimaryOutput()&&(0==(mPrimaryOutput->supportedDevices()&AUDIO_DEVICE_OUT_ALL_USB))){
                                             ...
                                             audio_session_t session)
{
    ...
    if (outputDesc->mPolicyMix != NULL) {
     ...
     if((!isFmMute) && (stream ==AUDIO_STREAM_ENFORCED_AUDIBLE || stream==AUDIO_STREAM_RING ||
        ...
        if((outputDesc->mRefCount[AUDIO_STREAM_ENFORCED_AUDIBLE]==0) || (outputDesc->mRefCount[AUDIO_STREAM_RING]==0) ||
                (outputDesc->mRefCount[AUDIO_STREAM_ALARM]==0) || (outputDesc->mRefCount[AUDIO_STREAM_NOTIFICATION]==0) || (outputDesc->mRefCount[AUDIO_STREAM_VOICE_CALL] == 0)) {
    ...
    if((!isVoipSet)&&(stream == AUDIO_STREAM_VOICE_CALL)&&(mPrimaryOutput->mIoHandle == output)) {
        ...
        if(outputDesc->mRefCount[AUDIO_STREAM_VOICE_CALL] == 1) {
            AudioParameter param;
            AudioParameter a2dp_param;
            param.add(String8("sprd_voip_start"), String8("true"));
            a2dp_param.add(String8("a2dp_voip_start"), String8("true"));
            mpClientInterface->setParameters(0, a2dp_param.toString());
            mpClientInterface->setParameters(0, param.toString());
            isVoipSet = true;
        }
    }
                                            ...
                                            audio_session_t session)
{
    ...
    ALOGD("stopOutput() isVoipSet %d,stream %d,output size %zd", isVoipSet, stream,mOutputs.size());
    if(isVoipSet &&(stream == AUDIO_STREAM_VOICE_CALL)&&(mPrimaryOutput->mIoHandle==output)) {
            ...
            if(outputDesc->mRefCount[AUDIO_STREAM_VOICE_CALL] == 1) {
                ...
                a2dp_param.add(String8("a2dp_voip_start"), String8("false"));
                mpClientInterface->setParameters(0, param.toString());
                mpClientInterface->setParameters(0, a2dp_param.toString());
                isVoipSet = false;
            }
    ...
    if (isStreamActive(AUDIO_STREAM_FM, 0) && (stream == AUDIO_STREAM_DTMF)) {
        AudioParameter param;
        int index = 0;
        getStreamVolumeIndex(AUDIO_STREAM_MUSIC, &index, outputDesc->mDevice);
                                       ...
                                       audio_session_t session __unused)
{
    ...
    if (testIndex != 0) {
        ...
        if (outputDesc->isActiveOrChanged()) {
...
//        if(device == AUDIO_DEVICE_OUT_FM_HEADSET){
        if(device == 0x10000000){
            ...
            if(state == AUDIO_POLICY_DEVICE_STATE_AVAILABLE){
                ...
                sp<SwAudioOutputDescriptor> outputDescriptor = mOutputs.getPrimaryOutput();
                if(true == isStreamActive(AUDIO_STREAM_FM, 0)){
