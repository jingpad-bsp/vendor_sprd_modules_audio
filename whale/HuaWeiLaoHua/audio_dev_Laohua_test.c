...
static int get_audio_dev_laohua_test_info(struct str_parms *parms,struct dev_laohua_test_info_t *test_info,int mode){
    ...
    if(IN_MODE == mode){
        if(ret >= 0){
            {
                test_info->fd  = open(value, O_RDWR | O_CREAT, 0666);
                if(test_info->fd < 0) {
...
void *dev_laohua_test_out_thread(void *args){
    ...
    AUDIO_TONE_GENERATE_STRUCT_T   *tone_test_struct =NULL;
    int ret = 0;
    ...
    switch_vbc_route(adev->dev_ctl,out_test_info->devices);
    select_devices_new(adev->dev_ctl,UC_NORMAL_PLAYBACK, out_test_info->devices,false,true,true, true);
...
void *dev_laohua_test_in_thread(void *args){
    ...
    switch_vbc_route(adev->dev_ctl,in_test_info->devices);
    select_devices_new(adev->dev_ctl,UC_MM_RECORD,in_test_info->devices,true,true,true, true);
...
int sprd_audio_dev_laohua_test(void * dev,struct str_parms *parms,UNUSED_ATTR int is_start,UNUSED_ATTR char * val){
    int ret=0;
