...
#define LOG_TAG "audio_hw_tester_client"
...
void send_error(const char *error)
{
    ...
    *(g_send_buf + index) = 0x7e;
    ...
    printf("m_head.len=%d\n", m_head.len);
    memcpy(g_send_buf + index, &m_head, sizeof(AUDIO_MSG_HEAD_T));
...
void send_command(int sub_cmd, int data_state)
{
    ...
    *(g_send_buf + index) = 0x7e;
    ...
    m_head.len = sizeof(AUDIO_MSG_HEAD_T) + sizeof(DATA_HEAD_T) + cur_len;
    printf("m_head.len=%d\n", m_head.len);
    memcpy(g_send_buf + index, &m_head, sizeof(AUDIO_MSG_HEAD_T));
    index += sizeof(AUDIO_MSG_HEAD_T);
