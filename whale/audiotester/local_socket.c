                          ...
                          uint8_t *buf, int len, int sub_type, int data_state)
{
    ...
    case AUDIO_EXT_TEST_CMD: {
        if(NULL!=res->dev){
            ext_contrtol_process((struct tiny_audio_device *)res->dev,(const char *)buf);
        }
        send_buffer(socket,AUDIO_EXT_TEST_CMD,DATA_STATUS_OK);
        break;
    }
    case SOCKET_TEST_CMD: {
        append_g_data_buf(_socket,(const char *)buf);
        send_buffer(socket,SOCKET_TEST_CMD,DATA_STATUS_OK);
        break;
    }
...
static int handle_received_data(UNUSED_ATTR void *_socket, uint8_t *received_buf,
                         int rev_len)
{
    int ret, data_len;
...
static void *listen_thread(void *args)
{
    ...
    if((connect_audiotester_process(local_socket,ser_fd,LOCAL_SOCKET_BUFFER_SIZE,handle_received_data)<0)){
...
int send_audio_command(int fd, int sub_cmd,char *cmd_buf,int cmd_size)
{
    ...
    send_buf=(char *)malloc(cmd_size+sizeof(AUDIO_MSG_HEAD_T)+sizeof(DATA_HEAD_T)+16);
    if(NULL==send_buf){
    ...
    *(send_buf + index) = 0x7e;
    ...
    m_head.len = sizeof(AUDIO_MSG_HEAD_T) + sizeof(DATA_HEAD_T) +cmd_size;
...
int receive_audio_command(int fd, char *cmd_buf,int max_size)
{
    ...
    DATA_HEAD_T *data_head;
    char *rece_buf=NULL;
    int rece_buf_size=max_size+2+sizeof(AUDIO_MSG_HEAD_T)+sizeof(DATA_HEAD_T);
    int ret=0;
    ...
    if(ret>=(2+sizeof(AUDIO_MSG_HEAD_T)+sizeof(DATA_HEAD_T))){
        m_head=(AUDIO_MSG_HEAD_T *)(rece_buf+1);
        data_head=(DATA_HEAD_T *)(rece_buf+sizeof(AUDIO_MSG_HEAD_T)+1);
        memcpy(cmd_buf,rece_buf+sizeof(AUDIO_MSG_HEAD_T)+sizeof(DATA_HEAD_T)+1,ret-(2+sizeof(AUDIO_MSG_HEAD_T)+sizeof(DATA_HEAD_T)));
