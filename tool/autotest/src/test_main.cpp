...
{
    int ret = 0;
    LOG_I("handle_audio_cmd_data:%s",buf);
    ...
    case HARDWARE_TEST_EXT_CMD: {
        ...
        if(0 == strncmp((const char *)buf,"setMusicVolume",strlen("setMusicVolume"))){
            ...
            if(ret<0){
                LOG_E("SendAudioTestCmd Failed ret:%d write again",ret);
                usleep(500*1000);
                ret=SendAudioTestCmd((char *)buf,len);
            }
            if(ret==len){
                LOG_E("SendAudioTestCmd Failed:%d %d",ret,len);
                ret=0;
             }
        }
    ...
    case DIS_CONNECT_AUDIOTESTER:
           test->wire_connected=false;
           LOG_I("DIS_CONNECT_AUDIOTESTER");
           break;
    default:
    ...
    while(true==test->wire_connected) {
       ...
       // ret = select(sock_fd + 1, &rfds, NULL, NULL, &tv);
        ...
        if(FD_ISSET(sock_fd,&rfds)){
            ...
            memset(test->audio_cmd_buf,0,rev_len+1);
        }
    }
    return NULL;
}
