...
int SendAudioTestCmd(char *cmd, int bytes) {
    ...
    int writebytes=0;
    if (cmd == NULL) {
        LOG_E("SendAudioTestCmd buffer is Null");
        return -1;
    }
    ...
    if (fd < 0) {
        ...
        LOG_E("open:%s failed error:%s",  AUDIO_EXT_DATA_CONTROL_PIPE,strerror(errno));
    }

    if (fd < 0) {
        LOG_E("SendAudioTestCmd buffer open pipe failed");
        return -1;
    } else {
        write_ptr=cmd;
        writebytes=0;
        do {
            ...
            if (ret > 0) {
                ...
                LOG_E("pipe write error %d, bytes read is %d", errno, bytes_to_write - bytes);
                break;
            } else {
