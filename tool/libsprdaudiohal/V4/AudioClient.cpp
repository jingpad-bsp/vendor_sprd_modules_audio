...
extern "C" {
       ...
       if(mDevicesFactoryHal){
          rc = mDevicesFactoryHal->openDevice(AUDIO_HARDWARE_MODULE_ID_PRIMARY, &primarydev);
           if (rc) {
               ...
               property_set("persist.vendor.audiohal.status","running");
           }
       }else{
    ...
    bool audioclient_check(void){
        char strval[PROPERTY_VALUE_MAX];
        property_get("persist.vendor.audiohal.status", strval, "");
        if(strncmp("died", strval, strlen("died")) == 0) {
            ALOGE("audioclient_check_audiohal died");
            audiocliient_init_status=false;
            ...
            int channels)
    {
        ...
        status = primarydev->openInputStream(
                handleIn, (audio_devices_t)devices, &in_config, AUDIO_INPUT_FLAG_NONE, streamName, AUDIO_SOURCE_DEFAULT, &inStream);
